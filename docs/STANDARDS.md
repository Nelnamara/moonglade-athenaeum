...
| **Claude Design** (claude.ai/design, Anthropic Labs) | Design-system projects that **round-trip with Claude Code** — import a design system, design with those exact components, hand off a bundle Claude Code implements from. Research preview, Pro/Max. | Harness has native support via the `DesignSync` tool (no dedicated skill by that name is present — drive the tool directly). Done 2026-07-29 on the `design-kit` branch: project **"Moonglade Athenaeum"** carries the kit (`kit/` — 8 `@dsCard` pages, 7 component JS files, `design-tokens.css`), regenerated by `tools/export_design_kit.py` and drift-pinned by `tests/test_design_kit_sync.py`. See docs/DECISIONS.md (2026-07-29) for the project id and the `gallery-top` merge note. |
