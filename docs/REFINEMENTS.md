...
| 17 | Loom Cast couldn't add an existing VIDEO from the gallery (owner-reported 2026-07-14) | ✅ | **Investigated + fixed.** `<mg-gallery-picker>` already had an opt-in `show-type` filter (Image/Video/All) that the Loom never enabled, and Cast's "+ add from gallery" hardcoded `kind:"image"` regardless of what was picked. Now: Cast's picker passes `allowType=true` (shows the type filter) and tags the new cast asset `kind:"video"`/`@video1...` when a video is picked (mirrors the existing `@image`/`@audio` tag convention) — these feed into `video_refs` for R2V/V2V shots exactly like a locally-uploaded video already did. FrameSlot's open/close-frame pickers were deliberately left image-only (a "frame" is a still). Browser-verified: type filter renders on Cast's picker only, picking a video tags `@video1`/kind "video" correctly. **Separate, NOT built yet, but CORRECTED SCOPE (2026-07-14):** attaching an existing video directly AS
a shot's *finished clip* (bypassing generation) — a real, standing roadmap ask (owner: discussed in
prior sessions, "not unreasonable to want to import an already-produced 5/10/15s video to the
timeline"). Checked `/api/loom/export` (the ffmpeg stitch job, `pixai_gallery.py` ~9439): it's ALREADY
fully generic — it takes `{clips:[{mid,in,out}]}`, resolves each `mid` via `get_row()`, and ffmpeg-concats
whatever video files are on disk. It has **no idea whether a clip came from generation or was
already sitting in the catalog** — the stitch path needs zero changes. The entire gap is one level up:
`resultMid` (`master-storyboard.jsx` `pollShot`, ~line 1303) is only ever written by a completed
generation task. **The actual fix is small:** a per-shot "use an existing video" action that opens the
(now video-capable, per the fix above) gallery-picker and calls `setCardStatus(cardId, {status:"done",
resultMid: mid, actualDur: <from the catalog row>})` directly — same shape `pollShot` already writes,
skipping `generateShot`/`pollShot` entirely. This is a real next-step candidate, not a "someday" — my
earlier "bigger feature needing its own scoping" was wrong before I'd actually read the export code. |
