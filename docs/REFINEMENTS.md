...
| 12 | Contest / community linkage — "the Oasis was never a 1-player game" | 📋 (un-parked) | **Recon 2026-07-04 un-parked this:** `contests(first:N)` returns the ENTIRE 63-contest catalog read-only over `gql_adhoc` (name/description/startAt/endAt/prize/rule/media) — NO capture needed. Build `--contests` mirroring `--account`; compute status client-side from dates; optionally match contest #hashtags vs the owner's own artwork tags for "contests I entered". None active as of 2026-07-04 (newest ended 2026-06-08) but live for future. Detail in `private/APP_OPERATIONS_FULL.md`. |
...
- 📋 **Per-artwork VIEWS + "Your Art" analytics** *(highest ROI, medium)* — `artwork(id){views}` reads live views on the owner's OWN art (views dwarf likes; we sync only likes/comments today). Add a `views` catalog column + extend `--sync-artworks`; surface a Your-Art panel.
- 📋 **Server backup-coverage stat** *(smallest, one-line query)* — `me{tasks{totalCount}}` = 19623 (server's lifetime gen count) → diff vs local catalog = true "X of 19623 backed up". Add to `_ACCOUNT_QUERY` + Health. Also grabs follower/following counts.
- 📋 **Contest catalog** *(medium — UN-PARKS #12)* — `contests(first:N)` returns all 63 in one page; a `--contests` read-only command mirroring `--account`. (None active as of 2026-07-04, but live for future.)
- 📋 **Notifications / engagement feed** *(medium, append-only)* — `notifications(first:N,type:LIKE|FOLLOW)` → "which of my images was liked, and when" (join by artworkId). HARD LIMITS: no actor identity; NOT a task-completion signal (do not repoint the Jobs tray at it).
- **DEAD ENDS (stop chasing):** real poll→push is a graphql-ws **WebSocket at wss://gw.pixai.art** — a separate transport, needs a new WS client + capture (big lift; opt-in watch daemon only). Comment bodies, who-liked identity, server-side "my/liked/saved models", and User aggregate rollups are all confirmed unreachable. REST for collections/stats/notifications all 404 (GraphQL-only).
