...
- **Full render-tree rebuild (classic+V2 merge): PARKED, not cancelled.** Undecided, no active work, would need its own probe.
- **State-layer consolidation (the composed-hooks extraction): SHIPPED and merged** (`ee4b33a`) — this is what actually happened instead of the rebuild.
- **V2 shell redesign (fixed 4-region layout): SHIPPED** (`c0c7399`) and independent of the rebuild question either way.
- **Three rounds of real bugs found live and fixed same day**, plus **draft generation** (Generate drawer works with no shot selected) **SHIPPED** on top of the shell.
...
**✅ TWO MORE FIXES, same day, third pass (owner asks B + E from the same message; commit pending):**
...
**✅ SHIPPED SAME DAY — Generate drawer works with no shot selected ("draft generation"), commit pending.** Owner asked, after loving the Generate flow: "I wonder if it should be gated behind selecting a shot?... the ability to click accept and move into the shot or to reject... Make sense? Or overkill." First answer given was wrong and got corrected on the spot — worth recording both the correction and why, not just the fix:
...
**📋 ROADMAPPED, owner-stated, not built — a full design/theme pass on the Loom.** *"Its going to be time to take a design and theme pass on the loom. We offer color schemes in the panel and those should apply to the loom as well. We are looking at a working windows now in need of live testing use and visual refinement."* Concretely: the Control Panel's skin system (Moonglade/Nightfallen/Moonlit Silver/Embercourt/Verdant Grove — cosmetic palette swaps, some earned via achievements) currently has no reach into the Loom at all; `V2_STYLES`' CSS custom properties (`--surface0/1`, `--accent`, `--text`, `--subtext`, etc.) would need to actually read from whichever skin is active instead of being hardcoded to one palette. Framed by the owner as the next phase once the shell is functionally solid: "a working window now in need of live testing use and visual refinement" — i.e. build/bugfix phase is substantially done, next phase is real visual polish + skin integration. Not scoped in detail yet.
...
Not yet built — this is a rebuild-scope design decision (the full render-tree unification, still parked per above), not something the shipped shell/draft-generation work below depends on either way.
