...
2. **Rail layout (progressive disclosure) — ✅ PARTLY SHIPPED (2026-07-14):**
   ...
   - **⚠️ NOT built — "center panels become dynamic reflowing panels":** deliberately left alone. This line is genuinely underspecified (what triggers a reflow? which panels? does manual drag/resize/collapse — which already exists and persists — count, or is something automatic wanted?). Didn't want to guess-build a layout auto-reflow system on my own interpretation, especially right after the "you built the Trophy Hall CSS with zero design input" friction. **Owner call:** define this one before it's built, or confirm the existing manual drag/resize/collapse (already persisted per-layout) is enough.
3. Folded-in mechanicals — **✅ SHIPPED (2026-07-14):** add/reorder/duplicate/delete shots AND acts · reparent `FrameSlot` into Generate (see §1 above). **Still open:** double-click deep-focus modal · snap-to-grid (neither started; small, well-defined, no design-input blocker — just not gotten to yet).
