...
**📋 ROADMAPPED, not built — owner request:** a larger text/button size option for the new shell. Owner's own framing: "the side panels look AWESOME but I have trouble making out the text... the color scheme isn't set and no theming yet though so I get it" — i.e. this is flagged as a real, near-term comfort/accessibility need, but explicitly not blocking since the shell's visual system (sizing, type scale, theming) isn't finalized yet. Revisit once a real design pass on V2's own visual polish happens, not before.
