...
- **the Control Panel** — the **⚙ Panel** overlay. Its tab is **Maintenance**; **Accounts**,
  **Trash** and **PixAI account** open as further windows on top of it. See [Control Panel](Control-Panel).
...
- **the narrator** — the voice that writes the one-line commentary on an earned card in the Folio.
- **overlay** — a window that opens on top of the gallery instead of taking you off it: the Folio,
  My Art, Contests, the Control Panel. **Esc** closes one and puts you back where you were.
- **the parade** — the run of achievement moments when several land at once, each taking the middle
  of the screen in turn. Also called the trail. **Esc** or **skip** ends it.
- **re-cut the pack** — rebuild the art pack, so a new release carries new art.
- **the reel bar** — The Loom's timeline strip in the Timeline drawer: one coloured segment per
  shot, sized by its duration.
- **Remix** — the **↺** control that loads a picture's whole recipe back into the dock. On the
...
- **Select mode** — the gallery's ticking mode. Ticked pictures feed the **Actions** dropdown, and
  the ticks survive turning the page.
- **the series popup** — the window that opens on a stacked card, with the session's runs listed
...
- **skin** — the palette the whole suite wears. Two ship free; the rest are earned; applied from
...
did nothing", or "the Control Panel's Trash tile showed no thumbnails". Naming the screen and the
button is usually enough to find it.

If a number looks wrong, add where you read it — the header, My Art, the Folio's **Statistics** tab
— because the same figure is shown in more than one place and they are read at different moments.
