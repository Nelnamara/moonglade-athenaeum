# 🌙 Moonglade Athenaeum Wiki

*A library against the Void.* — the full guide to backing up, browsing, generating, and curating your PixAI work.

New here? Start with **[Setup](Setup)** — it takes one API key and about a minute.

## Guides
- **[Setup](Setup)** — install + the one-key `config.json`.
- **[Backing Up](Backing-Up)** — download your history, incremental updates, organizing files.
- **[The Gallery](Gallery)** — browse, search, filter, rate, the lightbox.
- **[Generating Images](Generating)** — create images via the API (the web Generate drawer + CLI).
- **[The Loom](The-Loom)** — the storyboard surface: acts, shots, frame handoff, and video generation.
- **[Collections & Curation](Collections)** — Select mode, drag-paint, Collections.
- **[Deleting & Cloud Sync](Deleting)** — local vs cloud delete, reconcile orphans.
- **[Collection Health](Health)** — the analytics dashboard.
- **[Control Panel](Control-Panel)** — maintenance jobs, the scheduler, sync options, accounts.
- **[The Folio of Honors](Folio-of-Honors)** — achievements, ladders, points, and skins.

## Reference
- **[How It Works](How-It-Works)** — architecture, the API, why setup is one key.
- **[Trust & Safety](Trust-and-Safety)** — exactly what this tool can and can't do to your
  account, and the `READ_ONLY` flag.
- **[Troubleshooting](Troubleshooting)** — common errors and fixes.
- **[FAQ](FAQ)** — quick answers.

---

> **Runs against your own PixAI account.** Built to preserve and organize *your own* PixAI work — cheaper priority by default, claims your own daily free credits/stamina and auto-applies your free cards (video cards are tickets — a longer clip needs more, and when you're short the preview says the clip is charged in full; see [Free cards and videos](Generating#free-cards-and-videos)), destructive actions touch only your account. This is a public, multi-account-capable tool (see [Trust & Safety](Trust-and-Safety) for the login/access model) — "your own account" describes what each session can touch, not who can run it. Unofficial; not affiliated with PixAI.
