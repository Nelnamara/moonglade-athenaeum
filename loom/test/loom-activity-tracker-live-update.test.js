...
// rendered/still."
//
// [Port note 2026-08-08: static/mg-notify.js was DELETED in the React migration; the notify
// system now lives at gallery/src/notify/. The gallery-side machinery this file cites moved:
// the Jobs engine's poller is gallery/src/notify/jobs.js, the tray state machine
// (refresh/schedule) is gallery/src/notify/jobsStore.js, and installNotify() (index.jsx)
// still publishes window.JobsCard -- master-storyboard.jsx installs it at module scope, so
// the nudge target is unchanged. References below are retargeted; the argument is not.]
//
...
// but it left a gap: the notify system's OWN gallery-side poller (the Jobs engine's private
// poll, used via Jobs.track()) calls the tray's refresh() the INSTANT it sees phase==='done'
// or 'failed' (gallery/src/notify/jobs.js:66-67, trayRefresh) -- that immediate nudge is
// what makes the gallery's tracker look "live". The Loom's pollShot has never done the
// equivalent: it updates genState/setCardStatus (the per-shot badge, which the owner likes
// and this file does not touch) but never tells the shared Activity tray (JobsCard) to
// refresh itself. The tray is therefore only ever as fresh as its OWN independent,
// unsynchronized ~2.5-7s poll cycle (gallery/src/notify/jobsStore.js's refresh/schedule,
// 2500/7000ms) instead of catching up the moment pollShot -- the demonstrably live,
// real-time signal the per-shot badge already trusts -- learns the truth. That extra,
// unsynchronized hop is what let the two surfaces visibly disagree, and what let the tray
// read as frozen when that independent cycle lagged.
//
// Fix: pollShot's own tick() nudges window.JobsCard.refresh() on the SAME two branches
// the gallery poller does (done, failed) -- mirroring the already-working gallery pattern
// exactly, without adding any new poll loop and without touching the per-shot badge.
//
...
describe("pollShot nudges the shared Activity tracker exactly when it learns the real state", () => {
  test("the done branch calls window.JobsCard.refresh(), same as the gallery poller (gallery/src/notify/jobs.js)", () => {
