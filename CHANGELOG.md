...
- Fixed: the **bulk Actions menu can no longer run off the bottom of the screen.** Opened from the phone's bottom sheet, its tail items — the delete pair — could land past the edge with no way to scroll to them; the menu now flips above its button or pins itself on-screen, scrolling internally when space is tight. (2026-08-29) When the mark is a real (transparent) logo, the accent-colored square and centered "M" carried over from the original mockup used to peek through around it; the mark image now stands on its own, keeping its slow 3D tilt and sheen. (2026-08-27)
