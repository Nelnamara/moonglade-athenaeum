...
## [Unreleased]
- Under the hood: folding the last loose front-end scripts into the main app build. Done so far: the art-filters engine, the image picker, and the model/LoRA picker (gallery side). Also switched The Loom to its real pre-built bundle instead of transpiling itself in the browser — faster to load, drops a 3 MB dependency. No visible change to any of it.
