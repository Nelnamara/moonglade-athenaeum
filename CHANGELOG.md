...
- Fixed: **the living mark no longer drags the page sideways.** With the *Eclipse* mark animation picked, the shadow crossing the mark swung right out past the window's edge and the whole page grew a horizontal scrollbar that pumped in and out in time with the orbit -- on every screen, at every window width. The shadow now sweeps across the mark itself: same disc, same path, same speed, but it can no longer leave the mark's own bounds. Every other treatment on the roster was then checked the same way in a real browser, at both header sizes, and none of them moves the page. (One does sit past the edge without moving: *Moon dust*'s particle field is drawn wider than the mark on purpose, and reining it in would change how it looks -- so it is flagged rather than quietly re-cut.) (2026-09-04)
