...
## [Unreleased]
- **Free video cards now cover 10s and 15s clips** ([#15](https://github.com/Nelnamara/moonglade-athenaeum/issues/15)). They never did: a video card is a book of tickets and a clip costs one per 5 seconds (5s = 1, 10s = 2, 15s = 3), but the card check only ever knew single-ticket cards, so every clip longer than 5s went out with no card attached and was charged full price — even with a stack of V4.0 tickets in hand. The check now asks the way the website does, and the preview (drawer, CLI, and The Loom's cost line) reads **"uses N of H cards"** — what this clip needs against what you hold. When you're short, it says so honestly: *no card is used and the clip costs the full price* — nothing is partly applied — and, matching the website, Generate still spends if you click. The Loom's batch confirm counts each shot's tickets against your pool and names which shots will spend before you confirm. `--cards` shows the held count per card. Wiki: [Generating → Free cards and videos](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Generating#free-cards-and-videos), [Trust & Safety](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Trust-and-Safety).
