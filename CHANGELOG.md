...
- **A LoRA you pick now brings its trigger words with it — on the phone as well as the desktop.** Most LoRAs only wake up when their activation words are in the prompt, and until now the desktop drawer made you click **+words** to put them there while the phone offered no way to do it at all — so a LoRA added on mobile quietly did nothing to a picture you paid for. Picking one now writes its words straight into the prompt, the way PixAI's own composer does, and it will not write a word that is already there — so picking, un-picking and re-picking never leaves you with the same token three times. **+words** stays on both screens as the way back if you delete them on purpose, and removing a LoRA leaves your prompt exactly as you wrote it rather than cutting text out from under you. Restoring a recipe with **Remix** is untouched: it gives you the prompt that actually made the picture, word for word. (#45) (2026-09-04)
