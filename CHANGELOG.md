...
## [Unreleased]
- Under the hood: folding the last loose front-end scripts into the main app build. Done so far: the art-filters engine, the image picker, the model/LoRA picker, and the cost badge (the "this costs N credits / a free card covers it" line). Also switched The Loom to its real pre-built bundle instead of transpiling itself in the browser — faster to load, drops a 3 MB dependency. No visible change to any of it.
