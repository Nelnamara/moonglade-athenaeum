...
- **Looking at a view count adds one to it — so Moonglade looks rarely, and says so.** This is PixAI's own behaviour and there is no way around it: asking how many views a picture has registers a view. The old design asked twelve times every time you opened My Art, which means opening the panel to check on your most-watched pictures was quietly inflating them by twelve views a look. The sync asks once per published work, once per run, when you ask it to — and **`--sync-artworks --no-views`** refreshes everything except the view counts for a run where you'd rather not touch your own numbers at all. The blow-up rule subtracts the sync's own view before deciding anything, so a picture nobody looked at reads as nobody having looked. (2026-09-06)
