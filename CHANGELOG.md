...
- **The pickers show each model once, a search replaces the whole list, and the next page is on its way before you reach the end.** PixAI's trending feed hands the same model back on two different pages now and then, and since the 2026-08-08 rebuild the picker could not cope with that: a card behind a repeat could never be removed again, so it stayed at the top of every later list, on top of the real results — a few more with every search. That pile was "it's always the same LoRA" and "search fails": the matches were there, underneath. The list now holds each model once, and the next page loads about a page before you get to it instead of when you are already looking at the bottom, which is what the old picker felt like. (2026-09-07)
- **On an iPhone, the LoRA sheet has its head again: Models, LoRAs, the search box and Confirm selection.** Safari does two things no desktop browser does: it sizes `vh` against the screen with its toolbars hidden, and it confines a fixed panel that lives inside a scrolling area to that area. The sheet lived inside the phone's scrolling body and was sized in `vh`, so with the toolbars up it grew past the top of its space and lost exactly its head and search row, leaving the model list with no way out. The sheet and its dimmer now live outside the scroller and follow the visible viewport. (2026-09-07)
- **The picker's search shows what the site shows, your adult LoRAs included.** PixAI filters the market by what kind of client is asking, and the app asked as a backup tool, which gets the stricter policy meant for the mobile app: a search for a word that fills pages on pixai.art came back "No LoRAs match" here. The drawer now browses the market the way your own browser does, on the same key, so the same rows come back. (2026-09-07)
...
- **The Branding tab scrolls again.** Adding the font section pushed the sections past the bottom of the slab they sit in, and there was no way to reach them: the slab quietly shrank back to its own height and cut off whatever did not fit, so the Control Panel never saw anything hanging below and never offered to scroll. The slab keeps its full height now, and the panel scrolls to the end of it the way every other tab already did. (2026-09-07)
...
- Deleting a single image from PixAI now shows in the Activity window, like a bulk delete does. (2026-09-07)
