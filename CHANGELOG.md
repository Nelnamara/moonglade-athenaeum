...
- **The little marks around the app say what they mean now — and seven of them are drawn pictures instead of borrowed typing characters.** Most of the small symbols beside a label had been picked out of whatever the keyboard could reach, so the same shape ended up doing two jobs and some of them said nothing at all. Seven now have a real drawn icon: **search** wears a pair of binoculars *everywhere there is a search box* — the library bar, the command palette, the picture-search on the Folio, the duplicate list, the AI Tools list and the phone — a model's **use count** and the **USER LORA** badge each get their own, an **Entered** contest badge wears a check-badge on every screen that shows one (it was a star on five of them), the composer's **Snippets** button wears quote marks, the phone's **Folio** button wears a trophy where a moon used to sit, and **both doors into the Control Panel** — the phone's Control tab and the palette's *Control Panel* row — wear the same laptop-and-cog, so the Panel is one place rather than a gear that also means ordinary settings. The icons are drawn in the app's own colour rather than pinned to one, so they change with your skin, your hover, and a button that is greyed out. Six more marks are simply better characters: **✎ Edit this**, **⎙ Print sheet**, a purple **♛** on the AI tools that need a PixAI membership (it was a gold star, which this app uses for ratings), **⚔ counts toward The Arena**, **❖ unlocks … skin** beside the Folio's **⚑ unlocks a banner**, and **↩** on the undo buttons — Undo, Undo delete and Reset crop — leaving **↺** to mean Remix and **⟳** to mean Sync, which is what they always meant. The one mark that changed the most is **◈**: it means *lookalikes* and nothing else now, on every screen. Nothing moved, nothing was renamed, and nothing new is fetched from the internet to draw any of it. (2026-09-05)
