...
- **The Generate dock now matches its design.** When the new UI landed, the dock had been built loosely against its prototype and it showed. Brought back to the design of record, surface by surface: the composer and footer are the same on every tab (the ▲ settings toggle lives bottom-left on Image, Edit *and* Video, so the dock is no longer tiny until you switch back to Image), the "Pick a model first" notice sits inline instead of taking its own row, the prompt rests at a **roomier six rows** and grows with the text (the field scrolls past the room the dock has, the panel never does), and the runs strip **stays visible while you tune** in ▲ (tiles shrink a step) instead of vanishing. **Video** got its designed three-slab layout — Shot Mode with the reference banks, an Engine card that opens a palette over the real model roster with capability chips, Duration and Camera in the design's order, Mode & Channel — and a **Video prompt helper** switch wired end to end. **Edit** got its three slabs too — one Source strip shared by Edit, Fixer and Enhance (with the sub-tab strip inside it), Edit Model, Quality — plus Fixer's Region slab and Enhance's Art Filters slab. Where the prototype's *content* was demo data or not tab-aware, the app keeps real capability (the audio reference bank, mode-specific prompt hints, the real engine list, the real Fixer models); the design decides the pixels. Also fixed on the way: a second focus ring that was boxing the Image prompt (the composer's own brightened border is the focus treatment).
- Under the hood: the API-surface harvest tool now identifies a PixAI build by its **chunk set** rather than the CDN's version label — the label is reused across genuinely different builds, so a comparison against the old cache reported hundreds of phantom changes while quietly mixing chunks from several builds together. Each crawl now caches its own chunks, so an extract can never blend two builds, and a diff between crawls is a real diff.
- **Free video cards now cover 10s and 15s clips** ([#15](https://github.com/Nelnamara/moonglade-athenaeum/issues/15)). They never did: a video card is a book of tickets and a clip costs one per 5 seconds (5s = 1, 10s = 2, 15s = 3), but the card check only ever knew single-ticket cards, so every clip longer than 5s went out with no card attached and was charged full price — even with a stack of V4.0 tickets in hand. The check now asks the way the website does, and the preview (the Generate drawer's cost badge and the CLI) reads **"uses N of H cards"** — what this clip needs against what you hold. When you're short, it says so honestly: *no card is used and the clip costs the full price* — nothing is partly applied — and, matching the website, Generate still spends if you click. If your ticket balance can't be read, it says that rather than guessing. The Loom's batch confirm counts each shot's tickets against your pool and names the shots that may spend before you confirm (as an upper bound — a second card that also covers can still fund one at submit time). `--cards` shows the held count per card. Wiki: [Generating → Free cards and videos](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Generating#free-cards-and-videos), [Trust & Safety](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Trust-and-Safety).
- **Mirror to PixAI website** (Control Panel → Maintenance): an optional toggle that files each new generation into your pixai.art web **library**, not just your local backup. It rides your own logged-in browser session — read locally from Chrome/Edge/Brave (the token never crosses the network) and self-renewed, so there is nothing to paste — and files generations under the website's content policy rather than the stricter mobile-app one. Off by default; when off, nothing changes. `Connect` reads the session; the toggle switches it on (and can't be switched on without a session). `--mirror-check` verifies the renewal loop from the CLI without spending. Every spend guard still applies — READ_ONLY blocks it, a single create per submit, and it refuses (spends nothing) rather than silently falling back to the API key if the session is unavailable.
- **Image Details now shows the full generation surface** ([#18](https://github.com/Nelnamara/moonglade-athenaeum/issues/18)): the fields PixAI records but the catalog used to drop — inference profile, quality-tag prefix, prompt-helper state, control nets, LoRA parameters, priority, render time, backend, the task's own started/ended/updated stamps, retry count, moderation result, and (for video) mode and model. These are captured **as a generation happens**, not only when a later backfill comes past, so a freshly made picture arrives complete. Steps/Sampler/CFG that a model bakes in rather than recording per-task (Tsubaki.2 and other AuraFlow models) are now filled from the model's own preset — and where a model genuinely has no sampler or CFG, the em-dash stays, because that's the honest answer. Derived pictures and videos also record their source image and how they were made (edit/upscale/img2video) at capture time. Older rows gain all of this with `--backfill-full-meta --with-surface`.
- Under the hood: a full audit brought every catalog data-fill path in line with the above — the create-time row-builders (image, edit, and both video paths) now persist lineage and the full surface the same way the backfill does, a video's negative prompt is no longer dropped by `--sync-videos`, and `--sync` fills metadata before it resolves model names so a freshly pulled row gets its clean model label in the same pass rather than the next one. The full-metadata backfill now also checkpoints as it runs, so a large historical fill (a library of tens of thousands of images is that many task fetches, an hour-plus run) survives a Ctrl-C or a dropped connection and resumes where it stopped instead of losing the whole run.
- Fixed: a freshly generated image's Steps / Sampler / CFG now record the model's real behaviour rather than the numbers submitted alongside it. A model that runs on baked-in defaults and reports none of its own (AuraFlow models such as Tsubaki.2) now shows its true step count and a blank Sampler/CFG — the same values a backfill would fill and the same you'd see on a historical row — instead of the default step/CFG the request happened to carry. Ordinary models are unaffected (they report their own values, which still win).
- The internal development documents (decision ledger, art direction, house standards, the module-level architecture reference, and the Loom's original manual) have moved out of this repository into a private companion repo. Nothing user-facing is lost: the wiki remains the full public documentation, `wiki/The-Loom.md` absorbed the manual's remaining unique material and is now the one Loom manual (the in-app ? panel points there too), and the public `ROADMAP.md` continues to list pending features.
- Fixed ([#8](https://github.com/Nelnamara/moonglade-athenaeum/issues/8)): recovering a task that's still rendering now says so — "still 'queued' on PixAI — nothing to collect YET" — instead of "no media found", which read as a lost paid generation and sent debugging at a task that was fine.
- Under the hood ([#6](https://github.com/Nelnamara/moonglade-athenaeum/issues/6)): the video parameter builder now refuses to build a submit without an explicit quality mode — omitting it silently costs ~3x (PixAI's own default is the expensive one, measured by price probe). Every current path already sent it; this closes the trap for the next one, and the image-path comment no longer calls omission "always safe" without the price caveat.
- The Loom's top bar now fully wears the designed styleset: ▶ Generate all is the bar's one metallic accent button, the cost chip reads "≈ N cr to finish", the Draft and Mobile-view chips draw the design's own little check square instead of a browser checkbox, the storyboard switcher is its compact caret, Render carries the ⇧ glyph, and the title/hint text is gone — the bar opens straight with the controls, centered the way the design lays them out.
- Fixed ([#12](https://github.com/Nelnamara/moonglade-athenaeum/issues/12)): the Loom Mobile Draft chip's active glow finally shows. It was painted in the default gold while the rest of the chip re-colors with your skin — under a non-default skin that left a faint yellow smudge instead of a glow. It now glows in the skin's own gold.
- **↺ Remix** ([#4](https://github.com/Nelnamara/moonglade-athenaeum/issues/4)): load a picture's *full recipe* back into the Generate drawer in one click — right-click a thumbnail or use the new button in Image Details. Prompt, negative, size, steps/CFG/seed, the model at the **exact version** it rendered with, and its **LoRAs at their real weights**, recovered from the task itself by exact id (never guessed by name — a name collision could wire a different LoRA into a paid generation, so that path doesn't exist). It only fills the composer; generating is still your click, and the Generate button waits until the whole recipe has settled so an eager click can't submit half of one. Anything that can't be restored is said out loud: the "↺ from #…" chip turns amber and lists exactly what's missing. Desktop for now.
- The history reel's reuse click now restores the run's LoRAs and exact model version too, with the same disclosures — it used to restore everything *except* those, a disclosed gap since the spec.
- Achievement art is now genuinely earned: badge masters and per-achievement artwork only serve once their achievement is actually unlocked, instead of being fetchable early by anyone guessing addresses. The app's regular dress — the narrator, tracker art, banners, marks — is unaffected, and the Folio's locked tiles still show their art as designed.
- The achievements data no longer reveals how many hidden feats remain undiscovered: they now appear as a single mystery entry rather than one masked entry each, so the count can't be read out of devtools.
- Under the hood: the Control Panel Branding surface's customization slots are now exactly what the panel shows — the three banners plus Icons & marks. Mascot and reward art are no longer accepted as upload targets (they never appeared in the panel; the API has caught up with the decided boundary).
- The Gem Tome mark is retired from the mark roster (a 2026-07-23 ruling, finally executed). If it was your active mark, the app quietly falls back to the default — and it stays retired even on installs whose local files still carry it.
