...
- **Contests became a workbench: enter from the app, and track what you've entered.** Contest cards now open a real page inside the app — banner, prize pool, both dates with a countdown that goes gold in the last 48 hours, a preview of the entries, and the winners once results land. Entering happens in three places (a contest's page, a published piece in My Art, or right at publish time) and every one of them funnels through the same explicit confirm, because PixAI offers no way to withdraw an entry. A new **My entries** tab lists every contest you have a piece in with its deadlines and result dates, and a background sweep reads your entries and results back from PixAI — so an entry made on the website, or a result decided while the app was closed, still shows up. Desktop for now; the phone gets its pass next. (2026-08-31)
