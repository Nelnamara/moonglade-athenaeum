...
- Under the hood: a full audit brought every catalog data-fill path in line with the above — the create-time row-builders (image, edit, and both video paths) now persist lineage and the full surface the same way the backfill does, a video's negative prompt is no longer dropped by `--sync-videos`, and `--sync` fills metadata before it resolves model names so a freshly pulled row gets its clean model label in the same pass rather than the next one.
