...
- **The layout switcher is three small marks beside the SIZE slider now.** Picking masonry, grid or timeline used to mean opening the Filters tray and finding a full row of labelled buttons taking up its whole top edge — for a choice you make once and then live with. It is three glyphs in the header instead, ▤ ▦ ≡, sitting right next to the thumbnail-size slider they belong with; hover any of them and it tells you its name. The layout you pick is remembered the same way your thumbnail size is. **On a phone there is no switcher at all** — the phone has always laid the library out one way, and offering a control that changes nothing was worse than not offering it. (2026-09-04)
- **Looking for pictures that look alike is one thing now across the gallery on a computer, and it no longer takes your library away from you.** The ◈ mark is the door on every desktop surface that shows you a picture: on a card when you hover it, in the card's right-click menu, on the big viewer's button row, and on the detail page's row of lookalikes. Whichever one you press, the same thing happens — a small **◈ Similar to [the picture]** tag appears in the search bar with the number of matches beside it, and the lookalikes fill the library's own space. Press the tag's **✕**, or **Esc**, and your library is back exactly where it was: same search, same filters, same page, same place on the page, because none of them were ever touched to show you the matches. Any lookalike's own ◈ moves the search onto that picture, so you can follow a resemblance from one image to the next. The desktop detail page's **Find similar (model)** button is called **Filter by model** now — it never did find lookalikes, it filtered by the model that made the picture, and borrowing the word "similar" made two unrelated buttons look like a pair. **The phone is not part of this pass and is unchanged**: Similar there keeps its old **✧** mark, the picture screen's ✧ SIMILAR row finds lookalikes the way it always did, the big viewer's ✧ Similar still says its own mobile pass is coming, and that model-filter button still reads *Find similar (model)*. The phone gets its own pass later. (#43) (2026-09-04)
