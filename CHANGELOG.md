...
- **Looking for pictures that look alike is one thing now, everywhere, and it no longer takes your library away from you.** The ◈ mark is the door: it is on a card when you hover it, in the card's right-click menu, on the big viewer's button row, and on the detail page's row of lookalikes. Whichever one you press, the same thing happens — a small **◈ Similar to [the picture]** tag appears in the search bar with the number of matches beside it, and the lookalikes fill the library's own space. Press the tag's **✕**, or **Esc**, and your library is back exactly where it was: same search, same filters, same page, because none of them were ever touched to show you the matches. Any lookalike's own ◈ moves the search onto that picture, so you can follow a resemblance from one image to the next. The detail page's **Find similar (model)** button is called **Filter by model** now — it never did find lookalikes, it filtered by the model that made the picture, and borrowing the word "similar" made two unrelated buttons look like a pair. (#43) (2026-09-04)
