...
- Under the hood: **opening the catalog and upgrading it are two separate acts now**, and the web routes have stopped writing their own SQL (2026-08-23). Every read of your catalog used to re-run the whole schema-upgrade script first — forty-one statements, at every one of some thirty-nine places that open the file — so listing a page paid for an upgrade that had been finished for months. The upgrade now runs once, the first time a catalog is opened after the app starts, and every later read just reads; nothing has to remember to run it, so a library from an older release still upgrades itself on first touch exactly as before. Separately, eleven web routes had been reaching past the catalog helpers to write SQL of their own — the delete preview, My Art, the lineage panel, the History feed and the rest — and each is now a named question asked of one place. Nothing you see changes; the reads are the same reads.
- Under the hood: **the catalog helpers all open through that one door now, and the old side-hatch is bricked up** (2026-08-24). Yesterday's split gave the web a single way to open the catalog; the two-dozen-odd helpers the routes actually call — the readers and writers underneath them — still each opened a connection by hand through a deprecated shim and closed it in a `finally`. They all open through the one `catalog()` context now, which closes the connection for them and rolls back an uncommitted write on the way out exactly as before, so a helper that missed a close on some error path can no longer leak one. With its last caller gone, the `_connect` shim is retired. Nothing you see changes; the same reads and writes, opened one way.
...
- Fixed: the **cost badge and the button under it can no longer disagree**. The price you are quoted and the job that is actually submitted are now built once, by the same code, and handed to both — before, the badge worked one out and each Generate / Edit / Fix / video button worked another out for itself, so a quote could describe a slightly different job than the one you paid for. Picking a model by name in the Loom prices properly now instead of showing a permanent “couldn't verify the cost”, and on a read-only install a blocked generation stops **before** the app talks to PixAI at all rather than after. (2026-08-23)
...
- Fixed: every **Generate / Edit / Fix / Upscale** button now waits for a price that belongs to what you are about to submit. The guard that stops a settled "FREE" from carrying a *different*, chargeable job existed on the video drawer only; the other five cost lines each had their own hand-rolled copy of the price check without it. They now share one, so a setting changed a moment before you click can no longer ride the old quote — and after any submit the price is re-checked, because the tickets it just spent are gone. Two knock-on improvements: **Upscale** shows a price for pictures whose model your catalog doesn't know (it was quoting nothing while still offering the button), and an **Edit** instruction no longer blanks the cost and greys out ✦ Edit on every keystroke — an edit's price can't change with its wording.
