...
- **The phone's LoRA picker has a Done button, so it no longer reads as stuck after a pick.** Picking a *model* closed the sheet by itself, because you only get one. Picking a **LoRA** did not, because you can pick several — so the sheet just sat there, with no obvious way out but the small **✕** in its corner, which is a leftover from the desktop and is labelled with a key a phone does not have. The sheet's top-right is a proper **Done** button on a phone now, for both the Models and the LoRAs tab. Tapping outside it still closes it too. On a computer nothing changes: the **✕** and Esc are exactly as they were. (2026-09-07)
