...
- **Contest plumbing: entering a contest from the app, and the counters that notice.** The app can now submit an entry itself — always behind an explicit confirm, since PixAI offers no way to withdraw one — and a background sweep reads your entries and results back from PixAI, so entries made on the website (and a win decided while the app was closed) still count. No new screens yet; the contest surface itself comes with its design pass. (2026-08-31)
