...
- **The overlays reopen instantly, and Health no longer waits on a walk of your library.** Every nav overlay — Health, the Folio, My Art, Contests, the Control Panel, Publish — now paints the numbers and rows you last saw in its first frame and refreshes them behind, rather than showing an empty panel while it loads; anything that actually changes the library drops what it remembered, so a reopen after a publish, an import, a duplicate cleanup, a finished generation or a maintenance job always re-reads. Health's own measurement is kept on the server and re-taken the moment `catalog.db` changes instead of on a two-minute timer, is pre-taken at startup so even the first look is quick, and hands back the previous reading while it re-measures — so it is never stale and effectively never slow. Alongside it: the page no longer shifts sideways when an overlay opens, the backdrop blur is applied once the open animation has settled rather than re-computed every frame over the live grid (the resting look is unchanged), and contest cover art loads lazily. (2026-09-04)
