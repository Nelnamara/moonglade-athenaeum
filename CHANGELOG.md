...
## [Unreleased]
- **The Glossary now says which installs have the Identity strip and which have the ✦ Branding tab.** They are the same thing in two states, never both at once: the strip is what you get until **Under the Hood** is earned, and earning it replaces the strip with the tab. Five entries — the banner, the Control Panel, the Identity strip, the mark and skin — named one or the other as a fixed place with no condition, so on any given install at least two of them sent you to a screen that was not there. Each now says which state it belongs to. (2026-09-07)
- **Four Glossary entries described the app as it is not, and now describe it as it is.** The point of the page is that you can name a screen and a control in a report, which fails the moment an entry names something that is not there. **The Fixer** was listed as a live control on the dock's **Edit** tab: it is not built — the computer's Edit tab offers Edit and Enhance only, and the phone's Fixer sub-tab is a placeholder that says so — and the entry now says that plainly and points at the **Handfix** and **Face Enhance** presets under **Enhance**, which do that job today. **LAN session** told you to look for a 🌐 chip in the header; there is no such chip, so the entry now describes a LAN session the way you actually meet it, as controls that are simply not drawn. **Activity** sent you to the bottom-left of the gallery; the button sits at one end of the bar under the banner, and in The Loom's top bar. And **the detail page** now names its button by the label on it, **⧉ Copy prompt**. (2026-09-07)
- **↺ Remix stopped making twice the calls to PixAI it needs.** Finding out which quality modes a model offers takes a second look-up, and that look-up had been wired into the shared step every model and LoRA resolve goes through — so remixing a picture with four LoRAs asked PixAI eight questions instead of four, and the four extra answers were thrown away, because a LoRA has no quality modes to show. Only the screens that actually draw the mode bar ask for it now; everything else is back to one look-up. And a look-up that fails is remembered for a minute instead of not at all, so a model PixAI will not answer for stops re-asking on every keystroke in the price line. Nothing you see changes: the modes are still read for the composer, and when they cannot be read nothing is greyed out, exactly as before. (2026-09-07)
- **The phone's Create tab greys out quality modes a model does not offer too.** The greying shipped for the computer's generate panel only, so on a phone all five **Mode** chips stayed tappable whatever model you were on — and picking one the model refuses is not an error you ever see: the generation quietly re-runs on the model's own default tier, which is not the tier you were quoted for. The chips now dim exactly the way the bars do, say *Not offered for this model* when you hold one, and stay in place rather than disappearing, so the row keeps its shape. **Auto** is always available. It is the same single rule behind both screens, so the two can never disagree about the same model. (2026-09-07)
...
- **Poking the narrator quickly no longer rewinds what it says to you.** The narrator's warnings escalate line by line as you poke it, up to the fifth one that earns **Triggered** — but a tap that landed too soon after the one before it was answered with the FIRST line again, *“The narrator ignores you”*, however far along you already were. Those quick taps are the app reading one poke as two, and they never counted towards anything; now they say nothing at all instead of undoing the build-up. Your count was never affected — only what you were told about it. (2026-09-07)
- **Every real tap on the narrator counts again.** Two taps closer together than four-tenths of a second were read as one — which is well inside an ordinary rate of tapping, especially on a phone, so about half of a quick run of pokes was quietly thrown away and **Triggered** wanted roughly ten taps for its five. The app only ever needed to fold together the *same* tap arriving twice, which happens within a few thousandths of a second, so the window is now a seventh of what it was: faster than a hand, slower than a stutter in the browser. (2026-09-07)
- **The feat beacon's “30 a minute” limit now actually holds.** The three feats (**The Konami Code**, **Read the Manual**, **Triggered**) are announced to the server, and that announcement is capped so nobody can script their way to them. The cap counted per browsing session, and it identified a session by a marker it wrote into the sign-in cookie the first time it needed one — so a script that kept re-sending the cookie it was given at sign-in was handed a brand-new allowance every time, and the cap it was meant to hit never arrived. It counts against the sign-in itself now, and against the device's address, so re-sending an old copy of the cookie buys nothing. Nothing changes for normal use: earning all three by hand is nowhere near thirty calls a minute. (2026-09-07)
...
- **The generate panel greys out quality modes a model does not offer.** The five bars under **TUNING** were always all clickable, but the set a model actually accepts is its own: some offer Lite, Standard, Pro and Ultra, others only Pro and Ultra. Picking one a model refuses was not an error you ever saw — the generation quietly re-ran on the model's own default tier, which is not the tier the price you were shown was quoted for. A mode this model does not offer is now dimmed where it stands, and says *Not offered for this model* when you point at it. It is dimmed, never removed, so the row keeps its shape and you can still see what the model is missing. **Auto is always available**, and a mode that asks for a membership stays available too — that gate is PixAI's to enforce, not this one's. When the app cannot find out what a model offers, nothing is dimmed and the panel behaves exactly as it did before. (2026-09-07)
- Test coverage: dropped branding files end to end, the Control Panel's Mirror tile, and a fix for an order-dependent phone test.
- **Before a bulk delete, the preview checks each selected task on PixAI (up to 40) so the counts are exact; bigger selections say they are an estimate.** The dialog that shows you the whole blast radius of **Delete from PixAI** answered entirely from your own library, and your library cannot know about a picture you deleted on PixAI's website — so it counted images that were already gone among the files it was about to take. The delete itself always knew: it reads every task before it acts and leaves those files alone, because your copy is the last one anywhere. The preview now makes that same read. Select up to forty tasks and every one of them is read back from PixAI before the dialog opens: images PixAI has already dropped are counted separately, greyed out in the strip with a line saying they stay in your backup, and the headline number is what will actually be deleted. Select more than forty and nothing is read — the dialog says the counts are an estimate from your library, and that the delete still checks each task before it acts, which it does. A task that could not be checked — a blip, or no PixAI credentials on this machine — is counted from your library and labelled *not checked on PixAI*, never quietly written off: a read that fails can only make the dialog report **more** than will really go, never less. The reads run several at a time and give up after twelve seconds, so the dialog opens in a moment rather than waiting on forty round trips. Nothing is written and nothing is deleted by looking. (2026-09-07)
- **Closing the viewer lands you on the picture you were looking at; if you stepped onto another page, at the top of that page.** Stepping past the end of a page inside the full-screen viewer really does turn the page underneath it — so closing the viewer used to hand back a page you had never seen, held at the scroll offset of the page you left, dropping you in the middle of nothing. Close it now and the picture you were looking at is on screen: if it was already fully in view nothing moves at all, and if it was not, the library steps to it, instantly rather than gliding. Step onto another page while you are in there and you land at the top of that page, which is the only honest answer — there is no "where you were" on a page you have not read. A picture that has no card to land on — filtered out, or folded under a stack cover — leaves the library exactly where it is. This is the same promise the library has kept since 3.8.1, not a hole in it: nothing moves your view except your own hands, and closing the viewer is your own hands. Every other way out of the viewer — Edit, To Video, Lookalikes, the full record — is untouched and still goes where it always went. (2026-09-07)
- **The restart card's halo now pulses and glows together, cyan, on a quicker beat, with a bigger Nel.** When you restart the Athenaeum, the card that stands in while it comes back had a single slow emerald ring leaving a small mascot. The mascot is half again as large, the halo around it sits further out, and it now does two things on one beat: the ring still leaves it, in the Loom's cyan and reaching further, and the halo itself brightens and swells and settles — one beat, a touch quicker than before. Every line of the card's text is unchanged, the mascot still holds still rather than spinning, and a device set to reduce motion still gets no animation at all. (2026-09-07)
- **Batch cards open in the same popup as series, marked BATCH, instead of taking over the library.** With **Stack sessions** on, a card holding one generation's several outputs used to be a trapdoor: clicking it re-loaded your whole library as that one batch, and getting back out meant finding Clear. It opens the same popup a series does now — over the library, which does not move — and the popup says which of the two you are looking at, **BATCH** or **SERIES**, above the title. A batch reads *"— the batch"* and *"4 images · one generation"* where a series reads *"— the series"* and *"23 images · 4 runs"*. Esc closes it and you are back exactly where you were. The **View batch** button inside a picture's own record is a different thing and is unchanged — that one still filters the library to that picture's siblings, which is what it is for. (2026-09-07)
- **The phone's LoRA picker has a Confirm selection button, so it no longer reads as stuck after a pick.** Picking a *model* closed the sheet by itself, because you only get one. Picking a **LoRA** did not, because you can pick several — so the sheet just sat there, with no obvious way out but the small **✕** in its corner, which is a leftover from the desktop and is labelled with a key a phone does not have. The sheet's top-right is a proper **Confirm selection** button on a phone now, worded the way PixAI's own LoRA dialog words it, for both the Models and the LoRAs tab. Tapping outside it still closes it too. On a computer nothing changes: the **✕** and Esc are exactly as they were. (2026-09-07)
- **An update-available banner shows on every screen, desktop and phone, until the update is applied.** A new release used to announce itself once, as a single corner notice, on whichever screen you happened to have open when the hourly check found it — dismiss it, or simply not be at the keyboard, and the Control Panel was the only place left to learn there was anything to update to. There is now a strip across the top of every screen — the library, the composer, the Control tab, The Loom, the setup wizard — saying which version is ready, with **Update** on it. It stays until you actually update; there is no × that makes it go away. **Not now** folds it into a small pill in the corner of that one tab, which taps back open, and a reload brings the strip back. It never covers anything: the page moves down by exactly the height of the strip, notch and all on a phone. **Update** takes you to the same place it always was — the Control Panel's update window on a computer, the Control tab's update screen on a phone — and the strip itself still cannot install anything. (2026-09-07)
- **The phone's Control tab can apply an update, with the same steps and receipts as the desktop.** Updating was a computer-only thing: told a release was out while holding your phone, you had nowhere to go with it. The Control tab now leads with **Update available** when there is one. Tap it and you get the same card the desktop shows — which version you are on, which one you are going to, what the update will do to the server, and **Update now** — and once it is running, the same three steps with the same real timings and the same meter. A refusal reads the same way it does on a computer, in the tool's own words. The tab reloads itself when the server is back and tells you the update landed, exactly as it does on a computer. Nothing about who may update changed: it is the same signed-in-only action, with the same confirmation, on both. (2026-09-07)
- **The update banner stops covering the Loom and the full-screen viewer — everything it stands over now starts below it.** The strip pushed the ordinary screens down but sat straight on top of anything drawn as a full-screen layer: the Loom's top bar on both the computer and the phone (including the phone's only **← Gallery** link back), and the viewer's rating stars and its **Edit** / **To Video** / **Similar** chips. They were not just hidden — the strip took the taps meant for them. Every full-screen layer in the app now begins where the strip ends. The one thing still allowed over it is an achievement, which owns the screen for its few seconds. (2026-09-07)
- **On the setup wizard the banner tells you to finish setup instead of offering an Update button that did nothing.** The wizard is the one screen with nowhere to send the press, so the button pressed nothing at all and said nothing about it. It now shows the release and the words *finish setup first*, and no button. (2026-09-07)
- **Update from the Loom opens the update box on the first press, not the second.** Pressing **Update** on the Loom's banner carried you over to the library — and arrived with nothing open, because the press itself did not survive the trip, so you had to wait for the banner to come back and press it again. The press now travels with you. (2026-09-07)
- **Folding the banner to its pill and opening it again no longer leaves it overlapping the screen underneath.** The strip measured its own height a moment too early, while it was still growing, and published a figure about eight pixels short — so everything below it sat that much too high and the bottom edge of the strip lay over the header. It now measures the settled size. (2026-09-07)

- **The wiki now has a Glossary — the names for what is on screen.** Reporting something that went wrong meant describing it, because the parts did not have names you could look up: the dark blurred layer behind a popup, the panel at the bottom that makes pictures, the run of achievement moments when several land at once. There is a page for that now, 44 terms, one line each, and each line says where you meet the thing — so a report can say the screen and the button rather than the shape of them. It ends with the two lines worth following when something is wrong: name the screen, then the control, then what happened. Linked from the wiki's Reference list. (2026-09-07)
- **A file the delete dialog promises to keep is now kept, even if the second check fails.** When the preview reads your tasks back from PixAI and finds a picture PixAI has already deleted, it says so by name: that one stays in your backup, because your copy is the last one anywhere. The delete itself made its own, separate check a moment later — and if THAT one failed where the first had worked (a blip, a dropped connection), it had nothing to go on and took the file anyway, the very file the dialog had just promised would stay. What PixAI says is written down the moment the preview hears it, so the delete reads it back from your own library whether or not its own check gets through. Nothing is deleted by looking, and nothing changes if you cancel: what is recorded is what PixAI did to its own copy, which is true either way. (2026-09-07)
- **The delete dialog opens within twelve seconds, or says why it could not.** It reads every selected task back from PixAI first, and that read was supposed to give up after twelve seconds — but the limit only stopped NEW reads from starting. One stalled connection could sit there for the best part of two minutes with the dialog refusing to open and nothing you could do but wait. Each read now gets only the time that is actually left, the dialog stops waiting at the deadline, and any task that did not answer is marked *not checked on PixAI* exactly as a failed one is. If the app itself goes quiet, the dialog no longer hangs either: it tells you PixAI did not answer in time and falls back to the plain confirmation, which spells out what the delete will take. (2026-09-07)
- **The delete dialog's numbers add up.** Counting the already-gone files out of the headline left two sentences underneath that disagreed with it: *"4 files will be deleted"* over *"You picked 2; the other 3 come with their batches"* (which is five), and then *"1 of them is already gone"* — one of the four that were supposedly going, and staying, at the same time. The dialog now names the whole batch before it splits it (*"Those tasks hold 5 files in all: you picked 2, and the other 3 come with their batches"*) and says *"One more is already gone on PixAI"* — more, on top of the headline, not part of it. Pick nothing but files PixAI has already deleted and it says so outright rather than announcing that zero files will be deleted. (2026-09-07)
- **Closing the viewer lands you properly in Timeline layout too.** Timeline scrolls inside its own pane rather than scrolling the page, so the landing added on 2026-09-07 did nothing there: step onto another page inside the viewer, close it, and the timeline was still sitting where you left it on the page before. The landing now moves the pane the pictures are actually in, and "already on screen" is judged against that pane, so a card sitting below its fold is fetched back instead of being called visible. The other three layouts are unchanged. (2026-09-07)
...
- **← Gallery puts you back exactly where you were.** It went to the front of the library — so crossing over to plan a video and coming back cost you the page you were reading, the picture you had open, and your place on the page, every single time. The crossing remembers now: the page, the open picture and how far down you had scrolled all come back with you, whichever door you went in by — the header's **▰ The Loom**, the command palette, **Send to Loom cast**, or the phone's own **Open The Loom**. If you take hold of the page yourself while it is settling — a scroll, a touch, a key — it stops immediately and leaves the view where you put it. The phone's own door is not the poor relation: leave the library from a phone and the page, the picture and how far down that screen you were all come back too. A Loom opened straight from a bookmark, in a tab that was never in the library, goes to the front door as it always did, and says nothing untrue by doing so — and so does one opened in a **new tab** beside a library you left standing, which never got the chance to note where you were. The promise is for leaving and coming back in the one tab; if you keep the library open in another, that is where it still is. Your search and filters are still cleared, the same as on any other reload of the library. The link's wording and look are unchanged. (2026-09-06)
- **On a phone, the Loom opens its phone layout by itself.** It has had a real phone view for a while — the board and reel, built for a narrow screen — but it sat behind a checkbox in the desktop top bar, which is the one bar a phone cannot comfortably reach, so on a phone you met the wide four-panel tool and had to go hunting. A phone now gets the phone layout on arrival. **Tablets are untouched** and still get the full desktop tool, deliberately — the app's existing test for "this is a phone" is what decides, and a tablet has never matched it. Both switches still exist and both still win: untick **📱 Mobile view** to get the wide board on a phone, or tap **🖥 Desktop** in the phone bar, and that choice is remembered in that browser from then on — it is never a one-way trip in either direction. **A browser that had already used the old switch keeps whatever it was set to**; the new default speaks only for a browser that has never answered, so nobody is moved off a choice they made. The view is settled when the Loom opens and only those two switches change it after that — **turning the phone over does not swap the tool out from under you** mid-session, which it did while the layout was being re-decided on every repaint. The phone's own **Open The Loom** sheet describes the board-and-reel view you actually get, and points at **🖥 Desktop** for the wide board rather than telling you to rotate for one you were never given. (2026-09-06)
- **The Loom tells you what a project has already cost, not just what finishing it will.** The top bar has always carried an estimate of the credits still ahead of you — *≈420 cr to finish* — and nothing anywhere told you what the shots already rendered had actually cost. Only the account-wide credit history knew, and it doesn't know which project. A second figure now sits beside the first — *~1,180 cr spent* — and the two read as before and after. It is a **record, not a guess**: every finished shot is matched to PixAI's own charge for the generation that made it, the real number off the receipt rather than a price quoted beforehand, which is why it wears a different mark (**~** for what was spent, **≈** for what is still estimated). Hover it and you get the breakdown, act by act, with anything the number couldn't account for named out loud. **A re-rolled shot no longer forgets what the first try cost.** A shot remembers one clip, so re-rolling used to erase the only trace of the attempt it replaced — the money was spent, the picture was in your library, and the project's total quietly pretended neither had happened. Every superseded attempt is kept from now on and counted, so "spent" means spent. Attempts re-rolled before this existed can't be recovered, and the hover says so rather than letting you assume otherwise. **And it never rounds an unknown down to zero.** A shot whose charge PixAI never reported, and one whose picture has since been deleted from the library, are two different gaps — they are counted apart, shown as *(+3 unk)* beside the total instead of folded into it, and if the total is *only* unknowns it says **3 unpriced** rather than a confident and wrong **0 cr**. A clip you brought in from your library rather than rendered here is left out of the sum entirely — that money was spent somewhere else, on some other day — and the hover names it instead of silently dropping it, whether you brought it in through the Footage tab or attached it to a shot with **Use an existing video instead**, and whether or not anything on the board was ever rendered here at all. Two shots that came out of one generation are one charge, and the act that did not get billed for it says **counted in Act 1** rather than a **0 cr** that would mean it was free. Nothing is fetched from PixAI to work any of this out: it is one read of your own catalog, and it costs nothing. (2026-09-06)
- **The library keeps itself up to date now — the buttons stopped being the only way anything happens.** The Control Panel could hold exactly one automatic job on one cadence, so everything else waited for you to press it: the app was a living library that had to be watered by hand. The console now leads with **Runs itself**, a list of the jobs it performs on its own, and every row says the same four things — what it is, how often it runs, when it last ran, when it next will — with a **Run now** beside it, because a job that runs itself should still be one you can start yourself. **Sync now** every six hours, with the perceptual-hash backfill following it straight after. **Sync i2v videos** and **Top up Similar** nightly, **Reconcile deleted** weekly. Flip any row off, or change its *every*, and it is remembered. Three jobs that have no quick form — the full re-walk, the Similar rebuild, and rebuilding every thumbnail — are not on a cadence at all: they only run if they have not run in **sixty days**, a floor under a library left untended rather than a slot in a calendar, and a fresh install never opens by firing them. That floor is kept by the app itself rather than by the absence of a dial to lower it with: a schedule file edited by hand — or anything else asking — cannot put a full re-walk, a Similar rebuild or a whole thumbnail pass on a one-minute timer. Rebuilding every thumbnail also writes each new one beside the old one and swaps it in only when it is finished, so a rebuild interrupted half-way — now that it can start on its own, with nobody watching — cannot leave a half-written thumbnail sitting under a real name for the app to treat as done for the next two months. And if the older single automatic job is switched on and set to one of these same jobs, that row says so out loud — *the standing order below runs this one — this row waits* — instead of showing a cadence it is not keeping. **Nothing destructive is ever automatic, and cannot be made to be:** organize, undo organize, dedup and restore orphans have no row here and no way to get one — they stay buttons, with the same confirm and the same this-machine-only gate. And a job finishing never moves what you are reading: it announces itself in the corner and in **Activity**, and leaves your page, your search, your address and your ticked pictures exactly as they were. (2026-09-06)
- **A piece you publish stops going missing on your other machine.** Publishing was written only to the catalog of the machine you did it on — nothing anywhere ever re-read PixAI's own list of your published works unless you pressed **Sync published-artwork metadata** — so a piece published from the app could simply be absent from **My Art** on the other install, indefinitely. It is read back now: **every fifteen minutes**, once shortly after the app starts, and **the instant you publish, unpublish or re-tag anything**, so the machine you published from is right immediately and the other one catches up within the quarter hour. Titles, tags, likes, comments, aesthetic scores and visibility all come along with it. It is cheap because it stops early — it walks your published works newest-first and gives up after two pages in a row holding nothing new, the same "stop when you reach what you already know" the incremental pull has always used, so a quiet sweep costs one or two pages instead of your whole history. Works **younger than 90 days** have their like and comment counts refreshed on every sweep; **older** ones at most **once every 48 hours**, when the sweep walks the whole history once to collect them. Nothing you own is touched by any of it: it edits eleven columns PixAI owns, on rows that already exist, so a rating, a collection or a title you typed cannot be reached by it, and a work whose image was never downloaded is skipped rather than invented. One work PixAI describes in a shape this app has never seen is skipped with a note in the log, rather than costing the whole sweep the work it had already gathered. It stands aside while one of the big jobs is running — a sync, a full re-walk, a video pull — instead of writing the same rows out from under it, and it no longer repeats the sweep it does at start-up half a minute later. **And the full re-read stopped being able to undo your work.** Pressing **Sync published-artwork metadata** used to read your whole catalogue into memory, merge the new information into it, and write every row of it back — so anything saved while it was running, by you or by the app itself, was quietly rolled back to how it stood when the re-read began: a star you had just given a picture, a collection you had just put one in, on any row in the library, not only the ones it was updating. It now writes only the works it actually read, and only the columns PixAI owns, exactly as the fifteen-minute sweep does — so there is no window left in which it can undo anything. That was true before any of this: the re-read has always worked that way, and a library that now writes to itself every quarter hour is what turned an unlikely collision into a routine one. The **Sync published-artwork metadata** button is otherwise unchanged and still does the whole re-read on demand. (2026-09-06)

- **On a phone, Back closes what you opened instead of leaving the app.** Everything the phone opens over your library — a picture full-screen, a picture's own record, all six things in the **☰** menu (**My Art**, **Publish**, **Train a LoRA**, **Import**, **Contests**, **Collection Health**), the **Folio**, a printed contact sheet, the contest entry screen, and the three drill-ins inside **Branding**, the composer's **Advanced** and **Duplicates** — used to be invisible to the phone's own back swipe. You were reading a picture, you swiped back, and you were out of Moonglade entirely, on whatever you had been looking at before it. Every one of them now holds its place in the phone's history for as long as it is **open**, so one Back closes one thing and leaves what was underneath standing — and *open* means visible: several of these take a fifth of a second to slide away, and a second Back inside that moment is answered here rather than let out of the app while the screen is still on it. Back out of the Folio and the menu screen you opened it from is still there; back out of that and you are on your library. Nothing about the address changes — this is *depth*, not a place, so no link is created and nothing becomes bookmarkable. Looking at lookalikes has worked this way since it shipped and is unchanged; it is simply the first of many now rather than the only one. Bottom sheets — **Sort**, **Advanced Search**, **Actions**, the menu itself — are deliberately not included: a sheet already dims the screen and closes when you tap outside it, which is the way out a full screen has never had. And with nothing open, Back does exactly what it always did and leaves. (2026-09-06)
- **Turning the page on a phone starts you at the top of it.** Tapping **Next** halfway down your library opened page 2 already halfway down — the search box, the **All / Images / Videos** row and the entire first row of pictures scrolled away above a page you had not read a word of, on a page you had asked for. Both arrows land at the top now, every time, and so does stepping off the end of a page inside the full-screen viewer. A page refreshing itself in the background still moves nothing at all, which is the rule that has governed the library since 3.8.1. (2026-09-06)
- **Each of the three tabs keeps its own place.** **Gallery**, **Create** and **Control** shared one scroller, so reading deep into your library and then tapping **Create** dropped you into the middle of the composer, and coming back put you somewhere neither had chosen. Each tab is remembered separately now: leave the library deep and it is still deep when you return, and the tab you switch to opens where *you* left *it*. A pushed screen still parks everything underneath it, so the tab it was opened from is exactly where it was when the screen closes. (2026-09-06)
- **A sheet on the phone holds the page still behind it.** While **Sort**, **Advanced Search** or **Actions** is up, the library behind the dim is latched — it cannot slide around while you are being asked a question — and it is exactly where you left it the moment the sheet goes. The sheet's own list keeps its scrolling to itself. The picture record and the Folio got the same treatment: a flick that runs off the end of either now stops there instead of nudging what is behind it. (2026-09-06)
- **The phone respects the notch and the home bar at last.** The app has been written for years to leave room for a phone's rounded corners, sensor housing and home indicator — the tab bar's gutter, the footer under every sheet, the padding at the top of the picture record — and none of it was doing anything, because the page itself never asked the phone for those measurements. It asks now, on the gallery, on the sign-in page and in **The Loom**, so all of that room appears where it was always meant to. The gallery's own top row of buttons gets the same guard its bottom edges already had. On a phone without a notch, and in a desktop window, nothing changes at all. (2026-09-06)
- **The Folio's own title wears the drawn trophy.** The button that opens it took a real drawn trophy in the 2026-09-05 pass and the chip at the top of the screen it opens was still showing the typed one, so the same door said two different things. One trophy, both ends. (2026-09-06)

- **My Art tells you how your work is actually doing — every piece of it, not twelve.** The panel could only ever report on your top twelve published pictures, and it bought that report the expensive way: twelve separate questions to PixAI every single time you opened it, six at a time, which is why the panel was slow. All it kept from them was one summed figure labelled *VIEWS (TOP 12)*, and the per-picture numbers behind it were thrown away. **View counts are collected by `--sync-artworks` now, along with the titles and likes and comments it already collected**, so the panel reads them off your own machine and every one of the four things you could not have before is simply there: a real **lifetime views** total across your whole published library — counting an animated work once, not once for its still and again for its video; a **view count and a small comparison bar on every published card**, showing at a glance how each picture is doing against your best; a **Most viewed** sort beside Most liked; and the panel opening instantly, over your LAN, with nothing asked of PixAI at all. **Comment counts reach the cards too** — the number was already being fetched for every card and summed into the COMMENTS figure at the top, and had simply never been drawn beside the ♥ where it belongs. A picture the sync has not reached yet shows nothing rather than a confident zero, and when only part of your library has been swept the heading says so instead of passing a subtotal off as a total. (2026-09-06)
- **Moonglade tells you when something takes off.** A quiet picture that suddenly finds an audience used to be something you'd notice weeks later, if at all. When a sync finds one, a single notice appears in the corner — *"◈ Moonwell is taking off — +400 views in the last 24h, about 25× its usual pace"* — and that is the entire feature. It names the one work that is moving and counts the rest rather than listing them, because a notice naming six things is a feed. **It announces and does nothing else**: no view moves, nothing opens, nothing scrolls, and your library stays exactly where you left it. Once per sync, not once per page load, and not again after a reload. A picture is "taking off" when its pace over the window since the last sync is at least **three times its own lifetime average** *and* it has gained at least **25 real views** — so a busy picture needs a genuine surge rather than a good day, and a near-dormant one going from 3 views to 9 stays quiet. **A picture that had no views at all counts too**, and it is the plainest case there is: there is no usual pace for it to beat three times over, so the 25-view floor is the whole of the test, and the notice says *up from nothing* rather than quoting a multiple of zero. An animation is one picture here, not two — its still and its video share every number, and only one of them takes a place in the notice. The first sync of a library announces nothing at all: with no previous reading there is nothing to compare against, and a rule that fires on everything at once the first time it looks is a rule that means nothing. (2026-09-06)
- **Your followers and following are on screen.** Both numbers already arrived with every credit-balance check the app made, and no screen had ever shown them. They sit beside the credits figure in the header, and again in the Control Panel's **PixAI account** window. They report and nothing more — there is no follow button here, and following someone is a change to your PixAI account, which this surface does not make. When the account can't be read they show nothing rather than a confident zero, because *"nobody follows you"* and *"we couldn't ask"* are different sentences. (2026-09-06)
- **Looking at a view count adds one to it — so Moonglade looks rarely, and says so.** This is PixAI's own behaviour and there is no way around it: asking how many views a picture has registers a view. The old design asked twelve times every time you opened My Art, which means opening the panel to check on your most-watched pictures was quietly inflating them by twelve views a look. So nothing that runs unattended looks at all. The fifteen-minute sweep never reads a view count; a **scheduled** run of **Sync published-artwork metadata** skips them too, and the counts have their own row in **Runs itself** — **View counts**, once a week, on by default, doing the counts and nothing else. Turn that row off and the app never touches those numbers again. Pressing **Sync published-artwork metadata** yourself still reads them, because that is you asking, and **`--sync-artworks --no-views`** is there for a hand-run refresh where you'd rather not; **`--views-only`** is the opposite, the counts alone with no re-walk. With **`READ_ONLY`** set the view read is skipped everywhere, by hand included — it changes a number on your PixAI account, so it sits behind the same switch as publishing and deleting. A page that fails is reported as a failed page rather than quietly read a second time, so a sweep adds exactly one view to each work or says it did not finish. The blow-up rule subtracts the sync's own view before deciding anything, so a picture nobody looked at reads as nobody having looked. (2026-09-06)

- **On a phone, the finished picture in Activity opens when you tap it.** A generation lands, the notice appears in the corner, and the row in **Activity** shows its thumbnail — and on a phone that thumbnail did nothing whatever you did to it. It has mattered more since the library started standing still where you left it: that row is the phone's one way from *your image is done* to the image itself. It opens the picture now, the same as the same tap has always done on a computer — the Activity sheet steps aside and the picture's own screen comes up over the app. Nothing about the row, the notice or the library changes. (2026-09-06)
- **My Art says why it is empty.** The titles, tags and like counts on that screen all arrive with the published-artwork sync, so on a library where that sync has never run there is nothing for My Art to list — and it said *"Nothing here yet."*, which is also exactly what it says when you have genuinely published nothing. Two quite different situations, the same silence, with the answer to one of them a single button away. When the cause really is the missing sync it now says so and names the button: **Sync published-artwork metadata**, in the Control Panel under Maintenance — or `--sync-artworks` if you would rather use the command line. Every other kind of empty keeps the plain line: an Animations tab with no videos in it, a Visibility filter that matches nothing, a library that truly has published nothing. Those are all it can honestly claim. On the phone as well as the desktop. (2026-09-06)
- **Opening the banner back up is one movement.** Collapsing the hero banner down to its slim bar has always been smooth. Expanding it again was not: it jumped most of the way in a single frame and then slid the rest, two motions where there should be one. It grows in one now, from the slim row to the full hero, over the same half-second and along the same curve the collapse already used. In a narrow window the last stretch still settles at the end — there the banner is as tall as what is inside it rather than as tall as the window, and that is a height the movement cannot know before it starts. It is a shorter step than the jump it replaces, and in a narrow window the expand had no movement at all before this. (2026-09-06)
- **The restart mascot holds still.** Restarting the server from the Control Panel set the little mascot on the confirm card spinning, endlessly, for as long as the restart took — and it read as wrong rather than busy. It stays put now. The halo pulsing around it stays exactly as it was, and it is still what tells you the restart is under way; whether anything ever takes the spin's place is a question left open rather than answered with the first thing to hand. (2026-09-06)

- **"Delete from PixAI" on a single picture works on every picture now — including the ones it had never once worked on.** PixAI has two ways to delete, and which one it accepts depends on the generation: it will lift one picture out of a batch only while that batch still has another picture in it, and for anything else — a generation that made a single image, or the last image a batch has left — it removes the whole generation record instead, which is what PixAI's own website does. Moonglade only ever asked the first way. So on any generation that made one image, the button had been quietly refused by PixAI **every time since v2.5.0**, and on the last surviving picture of a batch it was refused too; you clicked, something failed, and the picture stayed. Now the app reads the generation back from PixAI at the moment you click and asks the way PixAI will accept. Nothing about this was a change on PixAI's side — the app had been wrong from the day the button shipped. And if that read fails — the network drops, PixAI is having a moment — **nothing is deleted at all**: an app that cannot see what it is about to delete does not get to guess. The same goes for a picture PixAI has already dropped, one its copy of the generation no longer lists, and a video clip, each of which now says so in a sentence instead of failing. (2026-09-06)
- **And it tells you the truth about what it is going to take.** The confirm dialog used to count the pictures sitting in your own library and word itself from that — *"the other 2 images in its batch stay on your account"* — which it had no way to know: a sibling you deleted on PixAI's own website is gone there while its row is still here. It asks PixAI first now and repeats **PixAI's** answer. If the rest of the batch really is still there, it says how many stay. If this is the last picture that generation has on PixAI, it says the whole generation record goes — and **names every file of yours that will move to your trash folder** with it, before you type `DELETE` rather than after. If the generation changes while the dialog is open, the delete is refused instead of doing the other thing. **Any images of that generation you had already deleted on PixAI are left alone** — their files and catalog rows stay exactly where they are. Those exist nowhere but your library, and a whole-generation delete does not change that; it can be one of them or several, and the dialog says how many. Deleting a picture also no longer leaves a way for it to come back: a re-sync used to list images PixAI had deleted and download them straight in again. (2026-09-06)
- **The Actions dropdown's "Delete from PixAI" leaves those same files alone now.** Deleting a selection takes each generation whole, on PixAI and here — and it used to take *every* local row of that generation with it, including the row of a picture you had deleted on PixAI's own website weeks earlier. PixAI has no copy of that one; yours was the only copy of it left anywhere, and a delete of its siblings quietly took it to the trash folder. It reads each generation back from PixAI once before it purges anything now, and any picture PixAI has already dropped keeps its file and its catalog row exactly where they are — the same rule the single-picture delete already followed. The Activity card says how many it kept, so the numbers add up rather than quietly disagreeing. If that read cannot be made — the network drops, PixAI is having a moment — nothing is purged that your library already knows PixAI dropped: a read that failed never widens what a delete takes. What PixAI deletes on their side is exactly what it was. (2026-09-06)
- **When your library finds out PixAI dropped a picture, it writes the date down there and then.** Every time the app reads a generation back from PixAI for a delete — the question the confirm dialog asks before you commit, the delete itself, and the Actions dropdown's **Delete from PixAI** — it records the date on the row of *every* picture of that generation PixAI no longer has, not only the one you clicked, off the answer it was reading anyway. That is what stops those pictures from being presented as ones PixAI still holds, and it costs no extra asking. `--backfill-full-meta` records it too, but only on rows it re-fetches — the ones still missing their prompt or their model details — so on a library you have already filled in, that pass records nothing and a delete is what teaches it. That filling-in is also a step of a plain sync, and **Sync now** is one of the jobs the app runs itself every six hours, so it can happen while you are not watching — on the same terms either way, only ever on a row that was being re-read for its missing details. Nothing else writes it at all: `--reconcile-deleted` works on whole generations that have left your feed and never looks at the pictures inside one, and neither the fifteen-minute published-artwork sweep nor the weekly view-count job touches it. Nothing is removed by any of this: a row carrying that date is one whose file is the only copy left anywhere, and it stays. (2026-09-06)
- **`--delete-task` on the command line is deprecated.** It still works this release and it says so when you run it, pointing at the two places deletion is looked after now: **Delete from PixAI** on a single picture, and **Delete from PixAI** in the Actions dropdown for a selection. Both of those check with PixAI first and move your local copies to the trash folder so your library and your account stay in step; `--delete-task` does neither — it removes the generation on PixAI and leaves the rows behind for `--reconcile-deleted` to find later. Nothing about how it behaves has changed while it lasts: still a dry run until `--apply`, still asking you to type `delete`, still one attempt per task. (2026-09-06)

## [3.8.1] - 2026-09-05 — By Hand
- **A picture finishing no longer throws you back to the top of your library.** You were on page 7, reading; something you asked for ten minutes ago landed — an edit, an enhance, a fix, a scene, a plain generate, an upscale, a video — and the gallery replaced what you were looking at with page 1. Nothing moves under you now: the page you are on, its address, the pictures on it and your place among them all stay exactly where they were, and the finished picture announces itself the way it already did — the notice in the corner, and a row in **Activity** whose thumbnail opens it. On page 1 the library still refreshes where it stands, because that is where a new picture arrives anyway: nothing moves there either, the new one simply appears at the top. **The three views you can be holding open are held still too**, at page 1 as much as anywhere: a picture open full-screen stays that picture rather than quietly becoming its neighbour, a set of lookalikes and a session stack keep the library beneath them exactly as you left it, so closing one still puts you back where you were. **And a page you asked for is the page you get.** Turning to page 2 the instant something lands used to be a race the finished picture could win — your page arrived and was thrown away, and the address slid back to the front of the library. Your own hand wins now, whenever you reach: the page you asked for is the page that loads, keeps its address, and stays. Deleting a picture from its own page behaves the same way — you come back to the page you were reading rather than the front of the library — and if it was the last picture on that page you step back one page instead of being thrown to the start. If you had pictures ticked, any that are no longer in front of you afterwards are un-ticked — after a page refreshes itself and after a delete alike — so a bulk action can never reach something you cannot see; ticking pictures across pages yourself is untouched. **And the phone has caught up.** It never noticed a generation finishing at all, so the credits figure sat at the pre-spend number and no achievement was ever checked; it does both now, under the same rule — the library only refreshes if you are already at the top of it, and never while you are looking at lookalikes. And the race is the phone's too: tapping **Next** the instant something lands used to lose the page you asked for, silently, because the phone keeps no address to argue with — the tap simply did not happen. Your hand wins there now as well, from the pager and from stepping off the end of a page in the full-screen viewer alike. (2026-09-05)
- **The little marks around the app say what they mean now — and nine of them are drawn pictures instead of borrowed typing characters.** Most of the small symbols beside a label had been picked out of whatever the keyboard could reach, so the same shape ended up doing two jobs and some of them said nothing at all. Nine now have a real drawn icon: **search** wears a pair of binoculars *everywhere there is a search box* — the library bar, the command palette, the picture-search on the Folio, the duplicate list, the AI Tools list and the phone — a model's **use count** and the **USER LORA** badge each get their own, an **Entered** contest badge wears a check-badge on every screen that shows one (it was a star on five of them), the composer's **Snippets** button wears quote marks, the phone's **Folio** button wears a trophy where a moon used to sit, **both doors into the Control Panel** — the phone's Control tab and the palette's *Control Panel* row — wear the same laptop-and-cog, so the Panel is one place rather than a gear that also means ordinary settings, every **Collection:** row in the command palette wears a shelf of books, and **every Undo in the app** — Undo, Undo delete, Reset crop, on the desktop and on the phone — wears the same clock winding back. The icons are drawn in the app's own colour rather than pinned to one, so they change with your skin, your hover, and a button that is greyed out. Five more marks are simply better characters: **✎ Edit this**, a purple **♛** on the AI tools that need a PixAI membership (it was a gold star, which this app uses for ratings), **⚔ counts toward The Arena**, **❖ unlocks … skin** beside the Folio's **⚑ unlocks a banner**, and **☁ Publish** in the phone's menu, which was the only Publish in the app wearing the *Edit* pencil. **Printing is one mark now.** Four buttons across the app all meant *print* and wore three different symbols between them — **▤** on the Actions menu (which is also the mark for the masonry layout), **🖶** on the contact sheet and in the Advanced flyout, **🖨** on a picture's own details. All four wear **⎙**. Two marks got their meaning back by being taken off everything else: **◈** means *lookalikes* and nothing else now, and **❖** means the Folio's skin flag and nothing else — leaving **↺** to mean Remix and **⟳** to mean Sync, which is what they always meant. Nothing moved, nothing was renamed, and nothing new is fetched from the internet to draw any of it. (2026-09-05)
- **The buttons that pick your mark show the marks.** In the Control Panel's Branding card — and worse, on the phone, where it was every one of them — a row of buttons that let you choose the mark beside the app's title drew a stand-in diamond instead of the actual picture, so five different marks looked like five identical tiles and you were choosing blind. Each button shows its own mark now, on both screens, and so does the little sample that pairs your mark with your skin. A mark whose picture is genuinely missing shows an empty dashed tile that says so, rather than a wrong picture or the blank button you used to get. (2026-09-05)

## [3.8.0] - 2026-09-05 — By Design

- **The art filters have a room of their own: the Darkroom.** They used to open in a panel floating over the drawer, with the two pictures squeezed into a third of it and the filter swatches crammed three-across in a column beside them — you were judging a filter on a thumbnail. The Darkroom takes the whole window: the filters run down a rail on the left — ours first, then PixAI's, each one showing its own colours — and your **source** and the **filtered** version sit side by side big enough to actually see. Strength and angle sit under them, **Esc** closes it, and it remembers the filter, the strength and the angle you were working with for the rest of the session, so stepping out to change the picture doesn't lose your place. Two buttons change: **Send to image gen** is now **Send to Edit**, because that is where it always sent you — it loads the filtered picture into the Edit tab as its source, and only the label was misleading. And **Publish** is gone: it never worked, it was drawn as the brightest button on the panel, and a dead button that looks like the main one is worse than no button. Publishing returns when there is something behind it. (#48) (2026-09-04)
- **The Enhance drawer is laid out like a drawer again.** Everything lived in one narrow column with an empty third of the drawer sitting beside it: the presets were two-across and tiny, the Generate button was buried halfway down, and a strip of filter swatches promised a picker it wasn't. It is two balanced columns now — the presets three-across in the wide half, your source and the **Enhance** button in the narrow one, with the button pinned to the bottom where you can always find it. The swatch strip and "Open compare view — M1–M7" are replaced by a single door, **Open the Darkroom ▸**. And the line beside the heading — *"Dispatches in seconds…"* — reads as one sentence at last: it was laid out as a row of separate pieces, so it came apart on screen into fragments with the spacing wrong. (#48, #44) (2026-09-04)
- **Change Emotion asks in families instead of handing you a wall.** Picking the expression for a Change Emotion run meant scrolling every expression PixAI ships in one undifferentiated grid, hunting for "the angry one". They are sorted into five families now — **Bright · Soft · Dark · Startled · Playful** — one family on screen at a time, four across, no scrolling: you pick a mood, then pick from a handful. The colour of each family rides the mark on the tile and nothing else, so the grid stays calm. If PixAI adds an expression this app has never heard of, it appears under **More** rather than quietly not being there, and a ★ still marks the ones that need a PixAI membership. (#49) (2026-09-04)
- **The AI Tools catalog leads with the picture.** Every tool sat in a six-across grid of small cards that all looked alike, and the thumbnail — the one thing that tells you what a tool actually does to a picture — was a thin strip above the name. The cards are three across now with the artwork on top at proper size, and the chip saying how much work a tool needs (**1-Click**, **Select**, **Text**, **Language**, **Dual**) sits on the art where you see it before you open anything. Those chips are colour-coded to the app's own scheme — one-click tools are the green ones, which used to be grey, the colour this app uses for "unavailable". Every tool still opens exactly as it did, and one with no artwork of its own still falls back to PixAI's, then to a lettered placeholder. (#53) (2026-09-04)
- **The Darkroom is usable in a narrow window.** In a window too narrow for the filter list and the pictures to sit side by side, the room folded into something you could not work in: the list of filters stood on end and ate the top third of the screen, the two pictures were sized off the width and spilled over their own space — painting straight across the sliders and the buttons underneath — and the third button dropped onto a line of its own. The same room now lies down properly. The filters become one strip along the top that slides sideways, each one still showing its own colours, with the group it belongs to marked at the left. Your **source** and the **filtered** version stack, sized to the room actually left so they always fit and can never cover anything. **Strength** and **Angle** each get their own line with their reading beside them, and **Save to library**, **Send to Edit** and **Reset to source** share one row across the foot, all three readable and all three reachable. Nothing changes in a normal window. (#48) (2026-09-05)
- **The AI Tools pictures are no longer cropped.** Each tool's card art was filled to the card, so a tall or square picture lost both edges and a face near the top of the frame could be sliced clean off. The whole picture is shown now, sitting on a soft blurred wash of itself so the card still reads as artwork rather than a picture stranded in a dark box. The card shape, the corner chip and the **Tier 1** badge are exactly where they were, and a tool with no artwork of its own still falls back to PixAI's, then to its lettered placeholder. (#53) (2026-09-05)
- **The layout switcher is four small marks beside the SIZE slider now.** Picking masonry, grid, hero or timeline used to mean opening the Filters tray and finding a full row of labelled buttons taking up its whole top edge — for a choice you make once and then live with. It is four glyphs in the header instead, ▤ ▦ ▣ ≡, sitting right next to the thumbnail-size slider they belong with; hover any of them and it tells you its name. The layout you pick is remembered the same way your thumbnail size is. **On a phone there is no switcher at all** — the phone has always laid the library out one way, and offering a control that changes nothing was worse than not offering it. (2026-09-05)
- **Looking for pictures that look alike is one thing now, everywhere in the gallery, and it no longer takes your library away from you.** On a computer the ◈ mark is the door on every surface that shows you a picture: on a card when you hover it, in the card's right-click menu, on the big viewer's button row, and on the detail page's row of lookalikes. Whichever one you press, the same thing happens — a small **◈ Similar to [the picture]** tag appears in the search bar with the number of matches beside it, and the lookalikes fill the library's own space. Press the tag's **✕**, or **Esc**, and your library is back exactly where it was: same search, same filters, same page, same place on the page, because none of them were ever touched to show you the matches. Any lookalike's own ◈ moves the search onto that picture, so you can follow a resemblance from one image to the next. The desktop detail page's **Find similar (model)** button is called **Filter by model** now — it never did find lookalikes, it filtered by the model that made the picture, and borrowing the word "similar" made two unrelated buttons look like a pair. **The phone speaks the same way now.** The big viewer's **◈ Similar** button was a message saying a phone pass was coming; it finds lookalikes for real, and they fill the library's own space under the same **◈ Similar to [the picture]** tag — in the phone's search bar, on its own line under the field so the field is still usable. The tag's **✕** puts your library back exactly as it was, and so does the phone's own Back gesture. The picture screen's lookalike row and the sheet it opens both read **◈** now instead of ✧, and its **Find similar (model)** button is called **Filter by model** there too. (#43) (2026-09-05)
- **Opening a session's stack of pictures no longer replaces your gallery.** A stacked card — a dial-in session where you kept re-rolling the same idea — used to swallow the whole library when you clicked it, and getting back meant hunting for Clear. It opens as a window over the gallery instead, and **Esc closes it straight back**: nothing underneath it moved, so there is nothing to undo. Inside, the session's runs are listed down the left as a descent line, each with its own picture count, headed by **All runs** — click one to see just it. Underneath them sit small chips for portrait, video and rated, which narrow whichever run you have picked rather than replacing it. The header sorts by **run №** or by **newest**, and every picture is stamped with the run it came from (r2·3 — run two, third picture) so the order you asked for is one you can actually read. An open stack has its own address, so it can be bookmarked and the Back button closes it. (#34) (2026-09-04)
...
- **Contests work on the phone now — you can read one and enter it without leaving for the website.** Tapping a contest on a phone used to do exactly one thing: open pixai.art in a browser tab. The Contests screen is the real thing now — the official contest as a wide picture at the top, the community ones as a list under it, and a **MY ENTRIES** count in the corner that turns that same list into just the contests you have a piece in, each saying where it stands. Opening one gives you PixAI's own brief, the prize breakdown and the entry requirements, folded so only the part you are reading is open and the rest still shows its headline figure, with the deadline and one **Enter this contest** button held at the bottom of the screen where your thumb is. Entering opens a full-screen picker: tap as many of your published pictures as you want and the bar underneath counts them and waits for you to confirm — **nothing enters on a single tap**, and the line at the top says which tag gets added and that the piece publishes to PixAI. You can start an entry from three places — a contest's own page, the full-screen viewer, or a published picture's details — and the last two arrive with that picture already ticked. The pixai.art link survives as a footnote on a contest's page, for when the website is what you actually want. (2026-09-04)
- **Fixed: scrolling a contest on the phone no longer drops you into the Control panel.** Open a contest, flick down past the brief, and the whole contest page slid away underneath your thumb — leaving you looking at the Control panel, with the **Enter** button seeming to hover over the wrong screen. What was actually scrolling was the tab you had left behind, which was still sitting under the contest page and still taking your flicks once the page itself ran out. A contest page now scrolls only itself, the tab behind it is held still while any menu screen is open, and **Enter this contest** is fixed to the bottom of the contest page it belongs to. The same fault could also open a menu screen apparently blank — if the tab underneath happened to be scrolled, the new screen was drawn off the top of the display; screens now always open at the top and the tab keeps its place for when you come back. (2026-09-05)
- **There are no contest entry fees, and nothing says otherwise any more.** Entering a contest costs nothing, but three screens still carried a cost line — the phone's picker said *"Entry fee unverified"*, and the confirm boxes on the desktop and at publish time had an **Entry cost** row. Every one of them is gone. (2026-09-05)
- **OFFICIAL and COMMUNITY are the same two colours everywhere now — and the official mark is gold metal.** The two words were coloured one way on the phone and the opposite way on the desktop, so the same label meant a different colour depending on which screen you were holding. They agree now, on the pairing the desktop board was already built around: an official contest is framed in gold there, so its **OFFICIAL** mark is gold too — a small brushed-metal tag with dark lettering, made the same way as the app's own metal buttons, catching the light slowly as they do. **COMMUNITY** takes lavender. If you have asked your device to keep motion down, the tag is still gold metal, it just holds still. Gold goes on meaning the prize, an entry you have made, and a win, on both screens. (2026-09-05)

## [3.7.2] - 2026-09-04 — Out of the Way

- Fixed: **"Update now" no longer refuses over stray files it was never going to touch.** The very first live update was turned away by a personal folder sitting beside the app — with the untrue explanation that the update would overwrite it, when nothing in the release lived at that path. The check now asks the right question of each kind of file: a **tracked file you have edited** still stops the update and is named, because the update really does write that path; an **untracked file or folder of your own** stops it only if the update genuinely needs that exact spot, and then says so honestly — *"an untracked file is where the update needs to write"*. Everything else — a notes file, a scratch folder, a launcher you keep next to the app — is stepped around and left alone, and the activity tracker records how many were passed over. (2026-09-04)
...
- **A LoRA you pick now brings its trigger words with it — on the phone as well as the desktop.** Most LoRAs only wake up when their activation words are in the prompt, and until now the desktop drawer made you click **+words** to put them there while the phone offered no way to do it at all — so a LoRA added on mobile quietly did nothing to a picture you paid for. Picking one now writes its words straight into the prompt, the way PixAI's own composer does, and it will not write a word that is already there — so picking, un-picking and re-picking never leaves you with the same token three times. **Removing a LoRA takes its words back out again**, exactly as PixAI does — so un-picking one doesn't leave you paying to generate against activation words for a LoRA that is no longer attached — while sparing any word a LoRA you're *still* using also needs, so dropping one of two that share a word never disarms the other. Your prompt closes up around the gap and is otherwise untouched: a word you reworded, or wrote into a sentence of your own, is your writing and stays put, and **+words** stays on both screens to put a LoRA's words back whenever you want them. Restoring a recipe with **Remix** is untouched: it gives you the prompt that actually made the picture, word for word. (#45) (2026-09-04)
...
- **The overlays reopen instantly, and Health no longer waits on a walk of your library.** Every nav overlay — Health, the Folio, My Art, Contests, the Control Panel, Publish — now paints the numbers and rows you last saw in its first frame and refreshes them behind, rather than showing an empty panel while it loads; anything that actually changes the library drops what it remembered, so a reopen after a publish, an import, a duplicate cleanup, a finished generation or a maintenance job always re-reads. Health's own measurement is kept on the server and re-taken the moment anything moves — a write to `catalog.db`, or a picture you delete or drop into the library folder by hand — instead of on a two-minute timer, with a ten-minute backstop so nothing can sit stale indefinitely; it is pre-taken at startup so even the first look is quick, and hands back the previous reading while it re-measures, so it is never stale and effectively never slow. Alongside it: the page no longer shifts sideways when an overlay opens, the backdrop blur is applied once the open animation has settled rather than re-computed every frame over the live grid — for every full-screen dimmed backdrop in the app, not just the shared overlay one (the resting look is unchanged) — and contest cover art loads lazily. (2026-09-04)
- Fixed: **the living mark no longer drags the page sideways.** With the *Eclipse* mark animation picked, the shadow crossing the mark swung right out past the window's edge and the whole page grew a horizontal scrollbar that pumped in and out in time with the orbit -- on every screen, at every window width. The shadow now sweeps across the mark itself: same disc, same path, same speed, but it can no longer leave the mark's own bounds. Every other treatment on the roster was then checked the same way in a real browser, at both header sizes, and none of them moves the page. *Moon dust* sat past the edge too -- its particle field was drawn wider than the mark on purpose -- and on the owner's call its field is drawn tighter now: the motes orbit closer to the mark and the page never grows the bar. (2026-09-04)
...
- **A command palette and a real keyboard layer for the desktop gallery.** `Ctrl/⌘ K` opens a search-anything palette — jump to the Library, the Loom, the Control Panel, Contests, My Art, Health, the Folio or any of your collections; switch layout or fold sessions into stacks; open the Generate dock, jump to search, run a sync, claim waiting credits; and, whenever a picture is focused, act on it with the same actions its right-click menu offers. Type to filter, arrows to move, Enter to run. Alongside it: `/` jumps to search, `N` opens the Generate dock, `G` then `L`/`S`/`C` goes to Library/Storyboard/Control Panel, `R` re-rolls the focused picture, `?` shows the whole map, and `Esc` closes whatever is on top. **`R` and "↻ Again — new seed" never spend anything** — they load the picture's own recipe into the composer with a fresh seed and stop there; you press Generate. Global keys stay out of your way while you're typing in any field. (2026-08-31)
- **The mark in the header is alive again — bigger, and it finally wears the animation you picked.** The Control Panel has always let you choose one and nothing ever showed it; now the header does, and the marks themselves are considerably larger (every surface that shows one grew to match). The roster is the ten that survived a two-round design pass, each rebuilt so it scales with the mark instead of looking loose at the new size — including **Moon dust**, a drifting particle field, and a **Glow** you can colour and aim. Two new sliders set the speed and size of whatever you pick, and the Control Panel's preview now shows the real thing rather than an imitation of it. Marks can also be **animated `.webp` files** now, so a mark you animate yourself just plays. (#24) (2026-08-31)
- **The sign-in page wears your Banner art.** The Branding tab's login slot has been croppable for a while but never actually showed up anywhere — the login card wore the small mark instead. Now the login banner runs full-width across the top of the card, above the title, exactly as the slot's 1920×480 crop is cut. A fresh install is dressed too: with nothing cropped yet, the shipped default art serves. (#25) (2026-08-31)
...
- Changed: **the video channel's second option now says what it does — "Private", works can't be published.** It read "Enhanced — for professional creators", which described a membership tier rather than the effect; PixAI renamed the same channel to Private, and the drawer, the Loom's per-shot Channel select and the CLI's help now match the site. Nothing about what gets submitted changed — the same field, the same behaviour, clearer words. (2026-08-31)
...
- Fixed: **collecting a generation twice no longer erases what you did to it.** If the app picked up a finished generation again -- a relaunch re-checking a task it had already fetched was enough -- it rebuilt that picture's catalog entry from scratch and wrote it over the top, blanking the published link, title, rating, collections, tags and score you had put there. (Found live: a published piece lost its link to the published artwork.) A finished generation is now recognized as already collected and left alone, and if anything does re-fetch it, your own fields survive the write. (2026-09-03)
- **A contest's page now tells you what the contest actually wants.** Opening one shows PixAI's own brief in full — no longer cut off mid-sentence — alongside a prize breakdown that reads tier by tier (what each rank pays, how many people place there, what the pool comes to) and the entry requirements: the tag your piece must carry, whether particular models or LoRAs are required, the rules document if there is one, and how the winners get chosen. You can read the whole ask and decide, without leaving for the website. (2026-09-03)
- **Contest entries are counted once, and every screen agrees which contests exist.** A review pass found the app reading the contest board four different ways, so a contest far enough down the list could be shown to you and then refused as unknown when you tried to enter it — and an entry could be recorded twice, nudging the contest badges along early. One shared reading of the board now serves every screen, an entry is recorded once by whatever made it, and a contest deadline shows the same date the rest of the app shows. Opening Contests no longer holds the window while it checks. (2026-09-03)
- **Honest words on the publish screen.** It used to say entering a contest at publish time was free; PixAI has never told us whether entry costs anything, so it now says so instead of guessing. A failed entry no longer claims nothing was submitted when the truth is that it might have been — it points you at My entries to check before you try again. (2026-09-03)

- **The overlays open fast again on a modest machine.** Folio, My Art, Contests, the Control Panel and Publish were each doing real work before they would paint — a full walk of your library's metrics, a live pull of the contest board, a complete contest sweep — on every single open. They now reuse a recent answer instead of recomputing one, the Folio's badges load as you scroll to them, and the Control Panel shows its frame immediately rather than nothing at all while it waits. Same numbers, same freshness where it matters; the wait is gone. (2026-09-03)

- **Contests became a workbench: enter from the app, and track what you've entered.** Contest cards now open a real page inside the app — banner, prize pool, both dates with a countdown that goes gold in the last 48 hours, a preview of the entries, and the winners once results land. Entering happens in three places (a contest's page, a published piece in My Art, or right at publish time — picking a contest while you publish now genuinely enters the piece, and says so if the entry itself doesn't land) and every one of them funnels through the same explicit confirm, because PixAI offers no way to withdraw an entry. A new **My entries** tab lists every contest you have a piece in with its deadlines and result dates, and opening it really does refresh from PixAI — so an entry made on the website, or a result decided while the app was closed, shows up on the next open rather than whenever something else happened to look. Desktop for now; the phone gets its pass next. (2026-08-31) And when a wave of achievements lands at once, they now play as a parade instead of one flat summary: each one takes the stage with its full moment, then settles into a shrinking stack behind the next, with a running count keeping score. (2026-09-03)
...
- Fixed: the **bulk Actions menu can no longer run off the bottom of the screen.** Opened from the phone's bottom sheet, its tail items — the delete pair — could land past the edge with no way to scroll to them; the menu now flips above its button or pins itself on-screen, scrolling internally when space is tight. (2026-08-29)
- Fixed: **your own mark shows cleanly in the header.** When the mark is a real (transparent) logo, the accent-colored square and centered "M" carried over from the original mockup used to peek through around it; the mark image now stands on its own, keeping its slow 3D tilt and sheen. (2026-08-27)
- Fixed: **signing in no longer cuts off its own send-off.** The mascot's pop-in and the "the gallery is waking" line now get their full beat before the library loads, instead of being skipped past the instant login completes (a real sign-in returns almost immediately, where the mockup's demo ran on a timer). A failed sign-in is unaffected — it drops straight back to the form with the error. (2026-08-27)
- **The new PixAI models price and generate correctly — the previewed cost matches what you're charged.** PixAI's 2026-08-25 platform overhaul split its image models into two families that take different generation settings, and the app was sending the wrong family's settings *and*, worse, quoting a price with one shape while submitting another — so the number you saw before generating could differ from what you actually paid. Image generation now sends each model family exactly the settings it expects, and sends the **identical** shape when quoting the price as when charging, so preview equals charge. (2026-08-27)
- Fixed: **Private videos stay Private on the new platform.** PixAI moved the video privacy switch to a new top-level field; the app now sends it — while still sending the legacy field too, so privacy can't regress — matching what the site itself submits. (2026-08-27)
- Changed: **image upscale (Hires) now runs 20 denoising steps** (was 26), matching PixAI's current default for the same visible result. (2026-08-27)
- Fixed: the **Generate drawer no longer feels sluggish**, and the price stops reloading as you type. The app was re-fetching each model's generation profile from PixAI on nearly every keystroke because the cache was keyed to a value that changed every request; it now remembers each model version's profile for an hour, so the drawer stays responsive and the price settles. (2026-08-27)
- **Start the artwork download from any device on your network.** The one-time art-pack download that dresses a fresh install used to be startable only at the server's own keyboard, so a phone or tablet that opened Moonglade *before* the PC ever did sat on an undressed library with no way to fix it — you had to go to the machine. A signed-in device on your network can now kick it off straight from its own Setup Wizard, and watch the real progress while it runs. Nothing else loosened: you still have to be signed in, it still fetches the one pinned, checksum-verified pack to the one place it belongs, and it still runs one download at a time — a second device asking simply joins the one already in flight instead of starting another. (2026-08-26)
- Security: some easter-egg triggers are now recognised only at the server's own keyboard. They were never worth anything — they light up a bit of flair and nothing else, they unlock no feature and spend nothing — but they could previously be nudged from any signed-in device on the network without the moment that earns them. Nothing you can *do* in Moonglade changed, on any device. (2026-08-26)
- Fixed: in the video Generate drawer, the **@image/@video reference chips no longer pile up** when the prompt field loses and regains focus — each focus-out used to wrap every chip in another copy of itself (a stack of icons deep, in the owner's QA recording), and a field corrupted by the old behavior now **repairs itself** on the next edit instead of needing a reload. The piled-up chips were visual only — the prompt a paid generation carried was always correct. Fixed alongside: hovering a chip's **thumbnail preview now opens beside the chip** instead of popping far away or off-screen (the dock's styling was hijacking its positioning). (2026-08-25)
- **Find Moonglade on your network — Bonjour/mDNS discovery.** The server can advertise itself on your LAN, so a phone, tablet, or any Bonjour-aware browser **sees “Moonglade” and taps it** — no more typing an IP — and the app owns its own name (`moonglade.local`) instead of borrowing the PC’s. A new **Bonjour** card in the Control Panel (under Server) is the one place to set it: broadcast on/off, reach (this PC vs the LAN), port, and the advertised name, with a live status line and a **Copy / QR** of the reachable address (scan it with the iPad). Broadcast and name apply instantly; changing the reach or port restarts the server. Off by default — it advertises only when you turn it on and the server is bound to the LAN, and it widens reachability only, never trust: a discovered `moonglade.local` still has to sign in. Needs the optional `zeroconf` package; without it the app is unaffected, it just doesn’t broadcast. (2026-08-25)
- Under the hood: the server now shuts down **gracefully** (a real `make_server`/`shutdown` rather than a hard process kill), so Stop/Restart flush cleanly — which is also what lets the Bonjour goodbye fire on the way out. Host, port, and the Bonjour settings now live in `config.json` (written by the chip), no longer hand-edited in the launcher’s `serve.txt`. (2026-08-25)
...
- **Stack a whole session into one card.** A new **Stack sessions** toggle (beside the layout picker) folds each dial-in series — and each multi-image batch — into a single **stacked card**, so the grid shows one cover per session instead of a wall of near-identical siblings. Each stack wears a badge (`SERIES · 4 versions · 12 images`, or `BATCH · 6`) and opens *into* just that session's members rather than the lightbox. Off by default; flip it back and every image is its own card again, exactly as before. Exporting a stack's CSV carries the stack, not the whole library.
- **Image Details, rebuilt to its design.** The picture sits still on the left; the record on the right is the only thing that scrolls — the page itself never does, and a wide image no longer stacks the record on top of itself. The headline is a quoted excerpt of the prompt (never the filename, which keeps its own ledger row); the main actions (Download, Publish, Copy prompt, Upscale, Delete locally) sit under the picture; the rest are grouped below the record, with the rarely-used ones in a quiet row at the end. An inline **Similar** strip shows the eight closest pieces by eye when the similarity index is built. Rarely-needed generation details fold away behind “More details.”
- **The card placard grew up.** The filename — word salad cut mid-word — is gone as the card's title. Each card now carries a small **accession stamp** (the day and time you made it, in your timezone, and the model), and a piece from a multi-image batch shows a **sibling strip**: tiny thumbnails of the batch's other outputs with the one you're looking at lit — so two siblings are finally telling apart at a glance. A real title you've typed appears above the stamp; nothing machine-written ever goes in that slot. The strip steps aside at the smallest thumbnail sizes. **View Batch** in Image Details works again for organized libraries.
- **The gallery remembers your page.** The address bar carries `?page=N`, so a bookmark, a reload, or the browser’s Back button returns you to the page you were on — including after closing Image Details, which used to drop you back to page 1. **Arrow keys** walk the grid (Up/Down go to the card actually above or below you), Enter opens the lightbox, and Right on the last card flips to the next page.
- **Lightbox: the picture is as big as it can be.** It fills the screen on whichever side is tighter, or shows at actual size if it fits — no more squares at half the screen. Publish wears the metal. And a portrait **video** no longer grows off the bottom of the screen.
- Fixed: dates now show **your** day, not UTC's. PixAI timestamps everything in UTC and the app was displaying that raw day, so anything you made in the evening filed under *tomorrow* — about half the library was a day off, and those pieces never landed in the Timeline's "Today" band. The card caption, the Timeline bands and Image Details all read the day in your own timezone now.
...
- Fixed: **a video you start on your phone now shows up in Activity** like everything else (2026-08-23) — it was rendering and billing perfectly, just invisible to the app's own job list, the Activity tray and the tidy-up that closes out abandoned tasks. And a long render now eases off its status checks on **every** surface at the same pace: every few seconds at first, then slower after twenty minutes, slower again after ninety, with the same "taking longer than expected" and "still going — unusual" notes wherever you started it. **Upscale** joins them: it was billing and delivering fine, but it never showed in Activity either — which is exactly what its own “watch it in Activity” message had been promising.
...
- Fixed: **Upscale no longer asks for a model** — it never should have, since PixAI's own upscale dialog has none (2026-08-23). The "choose a model" control is gone entirely: an upscale now always uses the picture's **own** model automatically, exactly like the site. And when there's no price to show, the cost line explains the **real** blocker — *"already at PixAI's size ceiling for Hires"*, say — instead of the misleading "the cost appears once this image has a model", which was the wrong explanation on a picture whose model was never what stood in the way. A related trap is closed too: a picture already at the ceiling can no longer be dragged into a paid same-size re-render by switching methods.
- Fixed: an over-ceiling **enlarge/upscale ratio is now named in the receipt** instead of being lowered in silence (2026-08-24). When your requested ratio tops out at the picture’s size ceiling — a 1400×784 source caps Hires at 1.4× — the adjusted-cost receipt now records *asked 1.9, used 1.4*, the same disclosure the width / steps / CFG / count clamps already make. What is submitted and billed is unchanged; only the disclosure is new.
- Fixed: **the cost badge and the button under it can no longer disagree**, and every cost line now waits for a price that belongs to what you are about to submit (2026-08-23). The price you are quoted and the job that is actually submitted are built once, by the same code, and handed to both — before, the badge worked one out and each **Generate / Edit / Fix / Upscale / video** button worked another out for itself, so a quote could describe a slightly different job than the one you paid for. The guard that stops a settled “FREE” from carrying a *different*, chargeable job used to live on the video drawer alone; all six cost lines share it now, so a setting changed a moment before you click can’t ride the old quote — and after any submit the price is re-checked, because the tickets it just spent are gone. Knock-on fixes: picking a model by name in the Loom prices properly instead of a permanent “couldn’t verify the cost”; **Upscale** shows a price for pictures whose model your catalog doesn’t know (it was quoting nothing while still offering the button); an **Edit** instruction no longer blanks the cost and greys out ✦ Edit on every keystroke (an edit’s price can’t change with its wording); and on a read-only install a blocked generation stops **before** the app talks to PixAI at all rather than after.
- Fixed: importing a task **by id** now says *already in your library — nothing new to fetch* when the task is already in your catalogue — on desktop **and** on mobile (the phone Control panel had still read the misleading “imported — 0 added”) — instead of the wrong count. And on mobile, **Create ▸ Video** and the **LoRA picker list** scroll again (both were stuck — a touch-drag couldn’t start a scroll). (2026-08-23..24)
...
- Fixed: **the browser tab icon loads again** across all three served shells — gallery, login, and **the Loom** (2026-08-24). They asked for `/branding/favicon.ico`, but no `.ico` bytes ship — the favicon is a plain `favicon.png` — so the tab icon 404’d on every page, including the pre-sign-in login screen; `rel=icon` now points at the PNG through the same public `/branding` route, and the mobile-web-app meta is current (the modern `mobile-web-app-capable` sits beside the retained Apple one for older iOS; the Loom shell carries no meta block, so none was invented).
- Fixed: the **login fields** (username, password, confirm password) carry accessible names now, so a screen reader announces each one. (2026-08-24)
- Fixed: **the grid's right-click menu was dead.** Every action on it — Edit, Send to Video, Remix, Find similar, Copy id, Open details — silently did nothing, because the menu closed itself on the very mousedown that was meant to fire an action, unmounting before the click could land (which is why "Remix from Details" worked but "Remix" on the menu didn't — Details' Remix is a separate button). The whole menu works again, and a **video's** menu gains **Rebuild poster** — the same on-demand poster re-extract as Image Details, now one right-click away. (2026-08-24)
- Added: **My Art marks a piece PixAI flagged as Sensitive.** An amber **Sensitive** pill now sits beside the Public/Private badge — read from PixAI's own moderation flag, not guessed — so a moderated work is no longer shown as a plain "Public". `--sync-artworks` backfills the flag for everything already in your catalog. (2026-08-24)
- Fixed: **the Animations tab was empty.** A synced video was tagging the *source image's* catalog row instead of its own, so nothing matched the Animations filter and animated works never showed; `--sync-artworks` now tags the video's own row (by its `videoMediaId`) and they appear. (2026-08-24)
- Fixed: **signing in on a deep link returns you to that exact page and image.** The front-door login redirect kept only the path, so opening something like `/?page=5&image=…` while signed out bounced you to page 1 after login — the query string is preserved now. (2026-08-24)
- Fixed: **the daily-reward claim keeps offering itself until you actually take it.** The claim popup used to show once and, if you dismissed it, never come back — not on a reload, not on returning to the tab — so a reward could sit unclaimed behind a toast you'd closed a single time. It now re-appears on a reload, on moving between the gallery and the Loom, and when you come back to the tab, and stops only once you claim. It still stays quiet as you move between in-app panels — no popup-on-every-click. (2026-08-24)
...
- Under the hood: **the catalog road — opening `catalog.db` and upgrading its schema are two separate acts now, and the web routes have stopped writing their own SQL** (2026-08-23..24). Every read of your catalog used to re-run the whole schema-upgrade script first — forty-one statements, at every one of some thirty-nine places that open the file — so listing a page paid for an upgrade that had been finished for months. The upgrade now runs **once per process**, the first time a catalog is opened, and every later read just reads; a library from an older release still upgrades itself on first touch exactly as before. Separately, eleven web routes had been reaching past the catalog helpers to write SQL of their own — the delete preview, My Art, the lineage panel, the History feed and the rest — and each is now a named question asked of one place. And the two-dozen-odd helpers underneath them, which had each still opened a connection by hand through a deprecated shim and closed it in a `finally`, now open through the one `catalog()` context that closes the connection and rolls back an uncommitted write for them — so a helper that missed a close on some error path can no longer leak one. With its last caller gone, the `_connect` shim is retired. Nothing you see changes; the same reads and writes, opened one way.
- Under the hood: **everything the app says to PixAI now leaves through one named door** (2026-08-23). There was no such door before: a bare connection object was handed down through some seventy functions, and the five pieces of code that actually spoke to PixAI each re-derived the rules from it — including the one rule that costs money, that a generation is submitted **once and never re-sent**. That rule now lives in a single place with no way to ask for the unsafe version, and it is checked from both sides. The test suite gained a stand-in PixAI that answers only what a test asked it to and refuses everything else by name, so "the tests never touch the network" is now built into the shape of the thing rather than maintained by hand in three hundred places. Nothing you do changes.
- Under the hood: **your account id now rides the connection instead of a slot the whole module shared** (2026-08-24). When the app connects to PixAI it works out which account the key belongs to; that id was being kept in one module-wide value every part of the code read from. The single helper that builds the history-feed request — the one that pages backward through your library — couldn't see the connection at all, so it had nothing to read but that shared value, which is what kept the value alive. It is handed the id now, taken from the connection the run authenticated as, and the four other places that read the shared value (the published-artwork list, the artwork-sync check, the account read-out, and the gallery's "my LoRAs" filter) take it off the connection too. What goes on the wire is identical — same account id, same request — and the value survives only as the place the connection is first told its own id. Nothing you do changes.
- Under the hood: **the last six PixAI reads now leave through the same one door as everything else** (2026-08-24). When the app's single connection to PixAI landed, most calls moved onto it, but six read-only lookups were left still hand-building their own request and speaking to PixAI directly — reading one task back, your bookmarked models, a model's name, a version's base model, a version's default sampler settings, and your published-artwork list. They ride the one client now too, so *every* PixAI call goes through the single place. The request each one puts on the wire is byte-for-byte what it was — same operation, same captured id, same variables (the published-artwork call's extra CSRF header included) — and the test suite's stand-in PixAI answers these six the same way it answers the rest, so the "tests never touch the network" guarantee now covers them by shape. Nothing you do changes.
- Under the hood: **the command-line create commands were measured against the app's one price/submit road and deliberately kept on their own** (2026-08-24). The web app builds, prices and submits every generation through a single piece of code, so a quote can't describe a different job than the one you pay for; the four command-line creators — image, image-to-video, reference-video and edit — were each read in full and checked against it, and left separate on purpose. They carry things that shared road has no way to express without changing what actually goes to PixAI: forcing a specific free card, handing in a raw parameter block verbatim (how `--task-id` recovery banks a submit shape), and their own size limits and model defaults — so routing them through it blind would quietly alter a real, paid submission. The result is a new test that pins exactly where they already agree (the plain case) and every place they don't, so a later attempt to merge them trips a guard that names what would have changed rather than discovering it on a spend. The commands themselves are untouched — same preview, same `--confirm`, same wire. Nothing you do changes.
- Under the hood: **the app asks what a generation costs from one place now** (2026-08-23). The Loom had nine separate hand-written copies of that request — the three Generate tabs, the Fixer, the cost-to-finish pill, the batch confirm — each with its own idea of what a dropped connection meant, and the gallery had a tenth. They share one now, and it draws the line the money depends on: a refusal the server actually sent is an answer, while a request that never arrived is "couldn't verify" — which is what turns the badge red and makes a confirm still ask before it spends. What each surface does with the answer is untouched, deliberately: the standing cost estimate still runs off its own warm cache, and "Generate all" still re-prices every shot fresh in the moment before you confirm. Nothing you see changes.
- Under the hood: **every part of the app that scans your library folder now uses the same scanner** (2026-08-23). Ten of them — the duplicate audit, `--organize`, `--import-local`, the Health page, the disk counter, resume, the Similar index and three more — each walked your folders with their own private list of what to skip, which is how a fix twice reached some of them and not others. They share one scanner now, and each still sees exactly the files it saw before; the places where two of them deliberately differ (the Similar index reads fewer file types, for instance) are kept and written down rather than quietly merged. Nothing you see changes, with one cosmetic exception: `--import-local`'s progress line now counts the media files it found instead of every file it looked at.
- Under the hood: **every ffmpeg and ffprobe call now goes through one place** (2026-08-23). Seven separate spots each decided for themselves whether the tools were installed, how long to wait for them and what to do when one failed — and they disagreed, including two that asked the same question about a clip's length and answered it differently. Video thumbnails, the faststart remux, the frame handoff, the Footage tab's duration and the Loom export now share one answer to each. "ffprobe isn't installed" is now something the app can *see* rather than something each spot happened to survive, so an export on a machine without it degrades on purpose, the way it already promised to. Nothing you do changes.
- Under the hood: the gallery front end now makes **every** call to the app's own server through one place, which decides once whether a reply is an error — the app's spending routes deliberately refuse with an "OK" status and an error in the body, and forty-six files had each been guessing at that rule for themselves, four of them from hand-copied helpers that disagreed. Nothing you do changes; failures that used to be swallowed silently in a few corners now surface. The library bar was rebuilt around the same idea — it takes the library as one thing instead of thirty-one separate pieces, and thirteen dead props that nothing had read in months are gone. (2026-08-23)
- Under the hood: **each web route now states its own permission level** — public, signed-in, or must-be-the-server-machine — right where the route is written, and one gate enforces all three. The permission used to be spelled out in a test file that mirrored every route by hand, with the strictest tier additionally re-typed as a check inside each of the handlers that needed it; a route could be added without one, or quietly lose one, and only a full test run would say so. Now a route with no stated permission stops the server at startup instead. Nothing about who can reach what changed — same refusals, same wording. (2026-08-23)
- Under the hood: **`catalog.csv` is retired as a moving part** (2026-08-24). The legacy auto-seed that rebuilt your catalog from a stray `catalog.csv` on startup is gone — it was a footgun (a stale CSV left in the folder could silently re-seed over the live `catalog.db`, which is the one source of truth), and it had never actually been needed. The command-line `--export-csv` dump goes with it. The gallery's **⬇ Download catalog (CSV)** browser download is untouched — that's a live, on-demand export, not a file the tool keeps on disk. Nothing you do changes.
- Under the hood: **the `--organize-adv` alias is gone** (2026-08-24). It did exactly what `--organize` does — a back-compat spelling with no reason left to exist; `--organize` and `--undo-organize` are unchanged.
- Under the hood: **the Similar index's embedded Postgres waits longer to start** (2026-08-24). `pixeltable_pgserver`'s start timeout was widened from 10s to 90s, so building the semantic-similarity index no longer fails on a busy machine where a cold Postgres takes a few extra seconds to accept connections.
...
**Updating an existing install:** first launch after this release downloads the new art pack (about 685 MB, verified before use) and moves the contents of your old `branding/` folder into the app's new arrangement — moved, never deleted; anything unrecognized stays where it was, and every move is logged. The paths the app asks for (`/branding/…`) are unchanged, and your own files still win over the packed defaults. If the Gem Tome was your active mark, the app falls back to the default mark — it's retired from the roster (a 2026-07-23 ruling, finally executed).

### The Bridge
- **Mirror to PixAI website** (Control Panel → Maintenance). An optional toggle that files each new generation into your pixai.art web **library**, not just your local backup. It rides your own logged-in browser session — read locally from Chrome/Edge/Brave and sent only to pixai.art, renewing itself — and files generations under the website's content policy rather than the stricter mobile-app one. Off by default; when off, nothing changes. **Connect** reads the session; the toggle can't be switched on without one. `--mirror-check` verifies the renewal from the CLI without spending. Every spend guard still applies: read-only mode blocks it, each submit creates exactly one generation, and if the session is unavailable it refuses — spending nothing — rather than silently falling back to the API key. If the server was down while you generated on the website, the mirror catches up on its own at the next start.
- **AI Tools — PixAI's 28 one-image "scenes", in the app.** Turn a character into a plushie, a tarot card, an SSR gacha pull, a magazine cover, an acrylic standee, a giant statue, a visual-novel screen, and more. A **✦ AI Tools** entry appears in the nav **only while the Mirror is on** (off, there's no entry and no hint) and opens a searchable, tier-tabbed catalog; pick a scene and the Generate drawer builds a **scene generator** for it live — one-click tools just Generate, others offer their real presets (tarot's Major Arcana, the fantasy classes), a language choice, a name/caption field, or a **second character**. Each runs on the Mirror — the only credential these tools work on — and the runs/History reel shows the result and its real cost. The scene list and every control come live from PixAI, so the catalog self-updates.
- **Enhance — PixAI's one-click workflows, behind the same gate.** Background removal, line-art, sketch coloring, hand-fix, face-enhance and change-emotion run only on a logged-in browser identity, so the Generate drawer's **Enhance** sub-tab offers them as selectable presets that work **while the Mirror is on** (off, the sub-tab says so and points at the toggle). All six were re-verified end to end; four had silently stopped working while still charging, and **Change Emotion** — the one preset with a control — was sending a malformed request (charged 2,000 credits, refunded within the hour, image unchanged). All six now work; Change Emotion offers the full **35-expression** set with the **25 that need a PixAI membership** marked. Each spends credits and **no free card covers a workflow task**, so the price shows before you generate. Wiki: [Generating → Enhance](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Generating).

### Creating
- **↺ Remix** ([#4](https://github.com/Nelnamara/moonglade-athenaeum/issues/4)). Load a picture's *full recipe* back into the Generate drawer in one click — right-click a thumbnail or use the button in Image Details. Prompt, negative, size, steps/CFG/seed, the model at the **exact version** it rendered with, and its **LoRAs at their real weights**, recovered from the task itself by exact id — never guessed by name, because a name collision could wire a different LoRA into a paid generation. It only fills the composer; generating is still your click, and Generate waits until the whole recipe has settled. Anything that can't be restored is said out loud: the "↺ from #…" chip turns amber and lists what's missing. The History reel's reuse click restores LoRAs and exact model version the same way.
- **↺ Remix for videos.** Video Details — and the dock's History and runs-reel video tiles — carry a working ↺ that loads a clip's full recipe (engine, duration, shot mode, camera, audio, prompt, and the reference frames) back into the Video tab, with the same amber chip for anything that genuinely can't be recovered. Fixed alongside: a finished video in the runs strip used to open the *image* recipe path and drop its prompt on the Image tab. Wiki: [Generating → The Generate drawer](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Generating#the-generate-drawer-web-gallery-v190).
- **History in the Generate dock.** The **History** button turns the runs strip into a **seven-day timeline read from your own catalog** — not just this session's jobs — one column per day, newest first, two rows deep, empty days saying "No runs", and **Load N older days ⌄** paging further back for as long as there is history. Tiles keep their real shape (a video tile is 16:9 with a ▶ tag); anything still cooking sits at the top with the mascot and a moving bar (PixAI reports no per-image progress, so no percentage is invented); hovering shows tag and time, model, size or clip length, the prompt, and **what it actually cost** — PixAI's own recorded spend, or "free card". Click a finished picture to load its recipe. History can be open alongside the ▲ settings, and Escape closes one layer at a time. A pure local read — no network.
- **The Generate dock matches its intended design on every tab.** The same composer and footer everywhere (▲ settings bottom-left on Image, Edit *and* Video), the "Pick a model first" notice inline, a roomier six-row prompt that grows with the text, and a runs strip that **stays visible while you tune** in ▲. **Video** gets its three-slab layout — Shot Mode with the reference banks, an **inline engine chip grid** in place of the old flyout picker (Camera now sits under Shot Mode, Duration under Mode & Channel), and a **Video prompt helper** switch wired end to end. **Edit** gets its three slabs too — one Source strip shared by Edit, Fixer and Enhance, Edit Model, Quality — plus Fixer's Region slab.
- **Free video cards now cover 10 s and 15 s clips** ([#15](https://github.com/Nelnamara/moonglade-athenaeum/issues/15)). A video card is a book of tickets — one per 5 seconds — but the card check only ever knew single-ticket cards, so every longer clip went out uncovered at full price. It now asks the way the website does, and the preview reads **"uses N of H cards"**. When you're short it says so: no card is used and the clip costs the full price — nothing is partly applied — and, matching the website, Generate still spends if you click. The Loom's batch confirm counts each shot's tickets against your pool and names the shots that may spend. `--cards` shows the held count per card. Wiki: [Generating → Free cards and videos](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Generating#free-cards-and-videos), [Trust & Safety](https://github.com/Nelnamara/moonglade-athenaeum/wiki/Trust-and-Safety).
- **The Loom** (the storyboard tool) wears its designed top bar in full: ▶ Generate all is the one metallic accent, the cost chip reads "≈ N cr to finish", the Draft and Mobile-view chips draw the design's check square, Render carries the ⇧ glyph, and the bar opens straight with the controls.

### The library
- **Four gallery layouts.** A picker in the Filters tray lays the same cards out four ways — **Masonry** (the original), **Grid** (uniform tiles; wide art spans two), **Hero** (the newest piece featured large every dozen tiles — never a video), and **Timeline** (date bands — Today, Yesterday, then by month — with sticky headers and a jump rail at the edge). Select, marquee, lightbox and every card affordance behave identically in all four; the choice is remembered. The thumbnail-size control became the app's own **Custom Slider**, now also the gallery's density control.
- **The Lightbox got its Classic feel back.** The fold-out prompt slab is gone — the lightbox is for looking, Image Details is for reading — so the picture fills the window (94% wide, 86% tall) the way the original did, and **double-tap zooms 2×** to inspect detail. Arrow-key browsing and the Publish/Upscale buttons are unchanged.
- **Image Details shows the full generation surface** ([#18](https://github.com/Nelnamara/moonglade-athenaeum/issues/18)). The fields PixAI records but the catalog used to drop — inference profile, quality-tag prefix, prompt-helper state, control nets, LoRA parameters, priority, render time, backend, the task's own started/ended/updated stamps, retry count, moderation result, and (for video) mode and model — are captured **as a generation happens**, so a fresh picture arrives complete. Steps/Sampler/CFG follow the model, not the request: a model that runs on baked-in defaults (Tsubaki.2 and other AuraFlow models) shows its true step count and a blank Sampler/CFG — the honest answer, and the same one a backfill fills. Derived pictures and videos record their source and how they were made. Older rows gain all of this with `--backfill-full-meta --with-surface`, which checkpoints as it runs and resumes after a Ctrl-C or a dropped connection.
...
### Fixed
- Two small reward icons that had been blocked by the very guard that keeps reward art unspoiled — a quiet, long-standing 404 — are served again.
- The first-run download screen's mascot ships inside the app itself, so it shows on the very first launch, before the art pack it's downloading has arrived. Its glow swells purple roughly in time with the animation.
- Honors no longer fire part-way through the first-run sync; they wait for it to finish and land as a real moment. Existing libraries are unaffected.
- ([#8](https://github.com/Nelnamara/moonglade-athenaeum/issues/8)) Recovering a task that's still rendering now says so — "still 'queued' on PixAI — nothing to collect YET" — instead of "no media found", which read as a lost paid generation.
- ([#12](https://github.com/Nelnamara/moonglade-athenaeum/issues/12)) The Loom Mobile Draft chip's active glow shows in the skin's own gold instead of a faint default-yellow smudge under non-default skins.

### Also new
- First-run setup: once the first sync has pulled in a few dozen items you can leave the sync screen with "Browse the gallery" — the sync keeps running in the background.

### Under the hood
- ([#6](https://github.com/Nelnamara/moonglade-athenaeum/issues/6)) The video parameter builder refuses to build a submit without an explicit quality mode — omitting it silently costs ~3×. Every current path already sent it; this closes the trap for the next one.
- Every catalog data-fill path now persists lineage and the full generation surface the same way; `--sync-videos` keeps a video's negative prompt; `--sync` fills metadata before resolving model names so a fresh row gets its clean label in the same pass.
- The API-surface harvest tool fingerprints a PixAI build by its chunk set, not the CDN's reused version label, so a diff between crawls is a real diff.
- The asset-container builder and the container's multi-block read path have real test coverage.
- The internal development documents moved to a private companion repo; the wiki remains the full public documentation, `wiki/The-Loom.md` is now the one Loom manual, and `ROADMAP.md` still lists pending features.
...
- **First-run setup fetches that file for you.** The Setup Wizard gains a one-time download step — real progress bar, checksum verification, and automatic retry across mirrors — before the rest of setup continues. If it can't reach the server you can retry or continue without the default artwork and pick it up later; the app is fully usable either way, and a later launch that's still missing the file routes back to finish the download on its own.
