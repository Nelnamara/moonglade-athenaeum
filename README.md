...
| **Browse & search** | Local web gallery: wildcard prompt search, model/LoRA/tag/rating filters, date pickers, lightbox, ZIP export, saved views, privacy blur · four layouts (masonry · grid · hero · timeline) · **◈ Similar** finds lookalikes by *eye* and fills the library's own space, handing it back untouched when you dismiss it · **session stacks** fold a night of re-rolls into one card that opens as a window over the gallery · the library stands still while you read it — a generation finishing announces itself in the corner instead of moving your page · mobile/PWA, contests included: read one, enter it and track your entries from the phone |
...
In-repo: [`CONTRIBUTING.md`](CONTRIBUTING.md) (how to contribute) — the deep material lives on the [Wiki](../../wiki) above, [How It Works](../../wiki/How-It-Works) included.
