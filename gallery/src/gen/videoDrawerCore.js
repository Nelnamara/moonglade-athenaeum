...
/* ---- price identity + the fetch timeout: MOVED to gen/priceProbeCore.js (2026-08-22) --------
   priceKey / canSubmit / PRICE_KEY_SKIP / PRICE_FETCH_TIMEOUT_MS were written here, for this
   drawer alone, after issue #15. They are now the shared price probe's -- every cost line in the
   suite rides the same gate (gen/priceProbeCore.js + gen/usePriceProbe.js), and the long WHY
   comments moved with them. Re-exported so this module stays the video drawer's one import
   surface and every existing import/test keeps working. Note the shared PRICE_KEY_SKIP also
   drops `seed`, which video payloads do not carry -- inert here. */
export {
  PRICE_KEY_SKIP, PRICE_FETCH_TIMEOUT_MS, priceKey, canSubmit,
