...
export default function ControlMobile({ account }) {
  ...
  /* ---- THE UPDATE, on the phone (owner ruling 2026-09-07: "phone gets update") ---------
     ...
     release was out and have nowhere to go with it. /api/update/apply is already
     @tier(LOGIN) and still takes its explicit CSRF token and its confirm, so this is a
     surface, not a policy change: the SAME useControlPanel() applyUpdate the desktop modal
     calls, drawn through the SAME UpdatePhases.jsx card, with the reload and the receipt
